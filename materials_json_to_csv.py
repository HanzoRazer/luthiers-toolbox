Here’s a JSON → CSV generator that’s the mirror of the CSV → JSON script you already have. It expects the same JSON structure your tests use:
...
1. Script: materials_json_to_csv.py
...
def convert_materials_json_to_csv(json_path: Path, csv_path: Path) -> None:
    """
    Convert a materials JSON file of the form:
...
2. How to use (round-trip workflow)
JSON → CSV (for editing in Excel)
python scripts/materials_json_to_csv.py \
  tests/data/saw_lab_materials.json \
  tests/data/saw_lab_materials_export.csv


Open saw_lab_materials_export.csv in Excel / LibreOffice, tweak densities, burn tendencies, etc.

CSV → JSON (rebuild fixtures after edits)
python scripts/materials_csv_to_json.py \
  tests/data/saw_lab_materials_export.csv \
  tests/data/saw_lab_materials.json


Now your pytest harness will run against the updated materials.