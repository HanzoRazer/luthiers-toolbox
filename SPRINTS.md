...
| **015-E-2** | `fretboard ecosphere` — `test_r2000_fret_slots_produce_grbl_gcode` | **SUSPECTED REAL-REGRESSION — test correct, kernel suspect** (the old 015-E "kernels correct / stale" framing does **NOT** carry to this half) | **OPEN — core-path (design→G-code).** Still RED on main after #125: GRBL pipeline emits **12 lines vs expected ≥100** for R2000 fret slots. Assertion names `_project_fret_slots` emission + the **2026-04-29 2-point loop-assembly gap** (a *lead*, not a confirmed finding). Fix = **kernel** (restore emission), **NOT** a test update — the test correctly expects ≥100. Grounding in progress (Unit 1a). |
