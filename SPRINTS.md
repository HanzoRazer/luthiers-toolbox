...
| **015-G** | **RMOS-persistence (11)** — `tests/cam/test_export_lifecycle_rmos.py` | **REAL-REGRESSION (kernel broken, tests correct)** — first cluster characterized in the three-way pass | **CLOSED by #132** (`c41fc656`). Root: orchestrator (`export_lifecycle_orchestrator.py:610`) set `audit_snapshot.rmos_summary` but `LifecycleAuditSnapshot` never declared the field → `ValueError`, crashing the governed export lifecycle (production-reachable: `export_lifecycle_router.py:68`). Fix = declare `rmos_summary` field. **Witnessed on Core CI run `27630309072`: the 11 cleared (0 remaining); suite 56→43 (−13, authoritative — NOT subtract-11).** #132 also cleared **2 overlapping tests** sharing the orchestrator crash (1 lifecycle-policy + 1 intent/op singleton). |
| **015-H** | **geometry-authority (10)** — `tests/cam/test_geometry_authority_references.py` | **NEVER-WIRED ROUTER** (not a regression, not a stale-test) — classification pending governance call | **OPEN.** Routes 404 (`assert 404 == 200`): `geometry_authority_router.py` (added `d5033799`, `prefix=/api/cam/geometry-authority`, full routes) is **never registered in any manifest** → unmounted. Test correctly expects 200. **Open question (governance, not technical): forgotten wiring (fix = add a `RouterSpec`) vs intentionally provenance-gated** (geometry-authority sits near `BLOCKED_PROVENANCE`; see `docs/MULTI_REPO_GOVERNANCE_CONVERGENCE_REPORT.md`). One sub-case differs: `test_validate_reference` → `KeyError: 'geometry_reference_id'`. Do NOT mount without confirming it's not a blocked feature. |
