...
| **015-E-2** | `fretboard ecosphere` — `test_r2000_fret_slots_produce_grbl_gcode` | **GROUNDED — TEST ON THE WRONG PATH; not a CAM kernel defect** | **OPEN — test-path issue, not a kernel fix.** RED because the test routes ecosphere fret slots through the GENERIC `dxf-to-grbl` adaptive-**pocket** planner (`compute_plan`, pocket-only `["Spiral","Lanes"]`, defaults `tool_d=6.0`/`strategy="Spiral"`); a 6mm tool can't pocket a 0.58mm slot → 12 vs ≥100 lines. **But production does NOT cut fret slots there.** The live path is the mounted `fret_slots_router` (`cam_manifest.py:155`) → `fret_slots_cam.generate_fret_slot_toolpaths` (ecosphere→G-code directly, **no DXF/R2000**), which already cuts slots correctly. `dxf-to-grbl` has **no production fret-slot caller**. **Refuted leads** (read the code): NOT the 2026-04-29 2-pt gap — emission writes correct 4-pt closed LWPOLYLINEs passing the `≥3` filter; `_project_fret_slots` doesn't exist. The generic planner "never cut thin slots," but that's not a defect to fix here — adding slot/profile to `compute_plan` would **duplicate** the existing `nut_slot_cam`/`profiling` capability (feature-parity violation). **Fix = re-point/reconcile the TEST to the dedicated path** (B1 re-point to `fret_slots_router`/`fret_slots_cam`; or B2 reframe/remove the format-compat guard whose 2026-04-29 premise is refuted) — sub-disposition pending. Making the generic `dxf-to-grbl` handle thin features = a **separate optional enhancement** (gated R2000/pro CAM), NOT required here. Evidence: `fret_slots_router.py:31,234`; `cam_manifest.py:155`; `mvp_router.py:27,35`; `plan_router.py:84`; `dxf_compat.py:117-123`; `fretboard_ecosphere.py:771`. |
