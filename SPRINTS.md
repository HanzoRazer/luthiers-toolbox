...
| CI-RED-015 | api-verify/API Tests: test-suite reconciliation — **56 failed** (API Tests run 2026-06-15; superseding the stale "72"), ~6 classes spread: body-solver/IBG (11), RMOS-persistence (11), geometry-authority (10), body-geometry-repair (4), lifecycle-policy (4), morphology-spine (3) + singletons. **Mostly uncharacterized — stale-test vs real-regression not yet established per cluster (do NOT assume stale-because-015-E-was).** Characterized so far: **RMOS-persistence (11) = REAL-REGRESSION → fix #132 (see 015-G)**; the rest pending. Characterization approach: `docs/sprints/SPRINT_SCOPE_CI-RED-015_characterization.md`. | CI / api-verify | OPEN | 2026-06-16 |
...
| **015-G** | **RMOS-persistence (11)** — `tests/cam/test_export_lifecycle_rmos.py` (TestRMOSPersistence / TestEndpointPersistence / TestNoMachineOutput) | **REAL-REGRESSION — kernel broken, tests correct** (first cluster characterized in the three-way pass) | **FIX IN #132** (characterized 2026-06-16). Single root for all 11 (verified — no second error): `run_governed_export_lifecycle` (`export_lifecycle_orchestrator.py:610`) sets `audit_snapshot.rmos_summary` on the persist-true path, but `LifecycleAuditSnapshot` (`cam_lifecycle_audit_ledger.py`) **never declared the field** → `ValueError`, crashing the governed export lifecycle. **PRODUCTION-reachable**: `routers/cam/export_lifecycle_router.py:68` is a live route → persist-true request 500s. Introduced by `3bd49bee` (7D contracts, half-implemented — orchestrator got the assignment, model never got the field). Fix = declare `rmos_summary: Optional[Dict[str, Any]] = None`. **CLOSES when #132 merges + the 11 go green on Core CI** (witnessed, per CI-RED-021's now-honest gate). |
