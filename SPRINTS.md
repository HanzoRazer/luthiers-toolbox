...
| **015-E-2** | `fretboard ecosphere` — `test_r2000_fret_slots_produce_grbl_gcode` | **GROUNDED — CAM param/strategy mismatch; NEVER-WORKED (not a regression, not stale-test)** | **OPEN — core-path, scoped to pro-tier-gated R2000 CAM.** `dxf-to-grbl` emits **12 vs ≥100** lines for 0.58mm fret slots: route defaults **`tool_d=6.0mm` + `strategy="Spiral"`** (pocket-clearing) — a 6mm tool can't enter a 0.58mm slot, so `compute_plan` emits ~no toolpath. **Refuted leads** (read the code, not the label): NOT the 2026-04-29 2-point loop gap — emission writes correct **4-point closed LWPOLYLINEs** via `dxf_compat.add_polyline(R2000)`, passing the `len(pts)≥3` filter; `_project_fret_slots` **doesn't exist** (assertion text only). **Never-worked, not a regression**: default has been `6mm/Spiral` since the route was first wired (`4cc640e4`) — no prior contour/profile default to restore. **Scope = R2000 (pro-tier) only**: R12/free inputs fail earlier (LINE not parsed → "no closed polylines"), not here; the test bypasses the tier gate via direct `write_ecosphere_dxf(version="R2000")` (legit unit-level, but scopes the red to gated R2000 CAM, **not** free-tier/ungated). **Fix = ADD thin-feature strategy selection** (slot/profile, or `tool_d ≤ slot_width`) on the R2000 CAM path — NOT emission, NOT a test change, NOT a default-restore. Evidence: API Tests on `568f23f6`; `mvp_router.py:27,35`; `dxf_compat.py:117-123`; `fretboard_ecosphere.py:771`. |
