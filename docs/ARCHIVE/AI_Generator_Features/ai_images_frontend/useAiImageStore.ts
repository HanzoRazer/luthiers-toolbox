...
export const useAiImageStore = defineStore('aiImages', () => {
  ...
  /**
   * Load assets from project (internal).
   */
  ...
  /**
   * Load assets from a specific project.
   */
  async function loadProjectAssets(newProjectId: string): Promise<void> {
    projectId.value = newProjectId;
    await loadAssets();
  }
  ...
  /**
   * Attach image to target (alias for attachToDesign).
   */
  const attachImage = attachToDesign;
  
  /**
   * Set gallery filters (replace all).
   */
   ...
   */
  function setFilter<K extends keyof GalleryFilters>(
    key: K,
