...
async function confirmReject(): Promise<void> {
  ...
  try {
    ...
    pendingAssets.value = pendingAssets.value.filter(a => a.id !== rejectingAssetId.value);
...
async function handleBulkApprove(): Promise<void> {
  ...
  try {
    ...
    pendingAssets.value = pendingAssets.value.filter(a => !selectedIds.value.has(a.id));
...
function toggleSelectAll(): void {
  if (allSelected.value) {
    ...
    selectedIds.value = new Set(pendingAssets.value.map(a => a.id));
  }
}
