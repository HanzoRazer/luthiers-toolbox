...
| 2026-05-21 | **Dev Order 67 Topology Variant QA & Measurement Lab Linkage Hardening complete** — Verified topology variant linkage integrity, sparse-state handling, archive-to-variant references, and observational comparison behavior. Added hardening utilities (isTopologyVariant, normalizeTopologyVariant, safeParseTopologyVariantTimestamp, hasTopologyVariantReferences, dedupeTopologyVariantReferences, filterArchivesByTopologyVariant, groupArchivesByTopologyVariant). 72 topology variant tests passing. Forbidden language audit added. Measurement Lab workflow remains stable. |
| 2026-05-20 | **Dev Order 66 Experimental Topology Variant Framework complete** — TopologyVariant type (topology-variant.v1 schema), TopologyVariantCard, TopologyVariantBuilder, collapsible Topology Variants section in Measurement Lab. Archives can reference topology variants via topologyVariantReferences. MeasurementResidualComparisonPanel shows shared/different variant status. In-memory only, observational only. |
| 2026-05-20 | **Dev Order 64 Measurement Lab Workflow QA & UX Stabilization complete** — Single source of truth for evidenceArchives, duplicate archive detection, sparse archive validation, safe timestamp parsing, forbidden words audit. 58 measurement archive tests passing. |
