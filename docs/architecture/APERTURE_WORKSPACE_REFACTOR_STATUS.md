...
| File | Change |
|------|--------|
| `packages/client/src/components/shared/acoustics/DiagnosticSnapshotCard.vue` | Tightened metadata gaps, added section separators |
| `packages/client/src/components/shared/acoustics/DiagnosticSnapshotExportMetadataCard.vue` | Added bullet prefixes to warnings |
| `packages/client/src/components/shared/acoustics/DiagnosticSnapshotImportCard.vue` | Tightened metadata/diagnostic spacing |

### Density Refinements

| Element | Change |
|---------|--------|
| Schema info grid | Gap reduced from 0.5rem to 0.375rem |
| Metadata grids | Gap reduced from 0.5rem to 0.375rem |
| Snapshot sections | Added subtle bottom border separators |
| Export warnings | Added bullet prefix (•) for scanability |
| Validation diagnostics | Margin reduced from 0.25rem to 0.125rem |
...
| Check | Result |
|-------|--------|
| `npm run build` | ✅ PASS (45.93s) |
| Component bundle | `ApertureComparisonPanel-DrNudUTi.js 98.80 kB` |
...
## Dev Order 51 — Snapshot Exchange Final UX Checkpoint

**Status:** Complete  
**Date:** 2026-05-14

Verifies Snapshot Exchange readability and behavior after Dev Orders 44–50 refinements.

### Verification Scope

| Area | Check |
|------|-------|
| Visual hierarchy | Snapshot (primary) → Export Metadata (secondary) → Import Validation (utility) |
| Warning ownership | Warnings appear once under "Export Warnings" label |
| Metadata chunking | "Schema" and "Export State" groups clearly separated |
| Validation flow | Result → Diagnostics → Action/Guidance readable |
| Observational notices | Present but not dominant |
| Export behavior | JSON download works |
| Import behavior | Exported JSON validates correctly |
| Language | No restore/upload/persistence/calibration wording |

### Build Status

| Check | Result |
|-------|--------|
| `npm run build` | ✅ PASS (37.95s) |
| Component bundle | `ApertureComparisonPanel-DpAdMhlf.js 99.54 kB` |

### Browser Verification

| Step | Expected | Result |
|------|----------|--------|
| Snapshot card reads as primary | Accent border, larger heading | ✅ PASS |
| Export Metadata reads as secondary | Muted styling | ✅ PASS |
| Import Validation reads as utility | Compact, functional | ✅ PASS |
| Warnings appear once | Under "Export Warnings" | ✅ PASS |
| Metadata grouping clear | Schema / Export State labels | ✅ PASS |
| Validation flow readable | Result → Diagnostics → Action | ✅ PASS |
| Notices readable but not dominant | Muted, separated | ✅ PASS |
| JSON export works | Download triggers | ✅ PASS |
| Exported JSON validates | Import shows valid | ✅ PASS |
| No forbidden wording | No restore/persist/calibrate | ✅ PASS |
| No console errors | Clean console | ✅ PASS |
...
## Dev Order 52 — Snapshot Exchange Documentation & Architecture Consolidation

**Status:** Complete  
**Date:** 2026-05-14

Consolidates Dev Orders 36–51 into a coherent architectural reference for the Snapshot Exchange subsystem.

### Deliverable

Created dedicated architecture document:

```
docs/architecture/SNAPSHOT_EXCHANGE_ARCHITECTURE.md
```
...
| Section | Coverage |
|---------|----------|
| Overview | Subsystem structure and purpose |
| Architectural Constraints | Observational boundaries |
| Snapshot Schema | Schema identifiers and structure |
| Export Workflow | Client-side download flow |
| Import Validation Workflow | Schema validation flow |
| UX Refinement Principles | Visual hierarchy, interaction philosophy |
| Warning Ownership | Final ownership rules |
| Known Non-Goals | Explicit exclusions |
| Component Files | File locations |
| Dev Order History | Complete Dev Order summary |

### Architecture Reference

**Full documentation:** [SNAPSHOT_EXCHANGE_ARCHITECTURE.md](./SNAPSHOT_EXCHANGE_ARCHITECTURE.md)

### Important

This order is documentation only. No code changes introduced.

---

## Dev Order 50 — Snapshot Exchange Information Chunking Refinement

**Status:** Complete  
**Date:** 2026-05-14

Improves information grouping and cognitive scanability inside Snapshot Exchange while preserving all existing snapshot/export/import behavior.

### Modified Files

| File | Change |
|------|--------|
| `packages/client/src/components/shared/acoustics/DiagnosticSnapshotCard.vue` | Added group labels (Schema, Export State), increased section rhythm |
| `packages/client/src/components/shared/acoustics/DiagnosticSnapshotExportMetadataCard.vue` | Added group labels (Schema, Export State) |
| `packages/client/src/components/shared/acoustics/DiagnosticSnapshotImportCard.vue` | Added group label (Schema), spacing break before action |

### Chunking Refinements

| Element | Change |
|---------|--------|
| Metadata groups | Added "Schema" and "Export State" inline group labels |
| Section rhythm | Increased label-to-summary spacing for cleaner units |
| Validation flow | Added spacing break between diagnostics and clear button |
| Notices | Added top margin to separate from primary content |
...
|------|----------|--------|
| Navigate to `/art-studio/aperture` | Route loads | ⏳ PENDING |
| Metadata groups visible | Schema / Export State labels | ⏳ PENDING |
| Section rhythm improved | Clear label-summary separation | ⏳ PENDING |
| Validation flow readable | Result → Diagnostics → Action | ⏳ PENDING |
| Notices separated | Top margin from content | ⏳ PENDING |
| JSON export still works | Behavior preserved | ⏳ PENDING |
| Import validation still works | Behavior preserved | ⏳ PENDING |
| No console errors | Clean console | ⏳ PENDING |
...
## Dev Order 48 — Snapshot Exchange Micro-Interaction Consistency

**Status:** Complete  
**Date:** 2026-05-14

Standardizes hover, focus, transition, and interaction consistency across Snapshot Exchange while preserving all existing snapshot/export/import behavior.

### Modified Files

| File | Change |
|------|--------|
| `packages/client/src/components/shared/acoustics/DiagnosticSnapshotImportCard.vue` | Standardized transitions, padding, opacity consistency |

### Standardization Applied

| Element | Change |
|---------|--------|
| File selector button | Added `box-shadow` to transition for focus consistency |
| Guidance padding | Harmonized from `0.5rem` to `0.375rem 0.5rem` |
| Error background | Standardized opacity from `0.1` to `0.08` |
| Diagnostics background | Standardized opacity from `0.1` to `0.08` |
...
| Background opacity | `0.08` for all diagnostic/validation panels |
| Padding (notices) | `0.375rem 0.5rem` |

### Important

This order changes interaction polish only. No snapshot semantics, export logic, import validation behavior, restore logic, or persistence behavior changed.

### Build Status

| Check | Result |
|-------|--------|
| `npm run build` | ✅ PASS (35.86s) |
| Component bundle | `ApertureComparisonPanel-vHrcMWJp.js 98.69 kB` |
...
|------|----------|--------|
| Navigate to `/art-studio/aperture` | Route loads | ✅ PASS |
| Hover timing consistent | Unified 150ms | ✅ PASS |
| Focus timing consistent | Unified 150ms | ✅ PASS |
| Validation panels aligned | Same structure, different tints | ✅ PASS |
| Cursor behavior correct | pointer on interactive only | ✅ PASS |
| JSON export still works | Behavior preserved | ✅ PASS |
| Import validation still works | Behavior preserved | ✅ PASS |
| No console errors | Clean console | ✅ PASS |
...
| Date | Change |
|------|--------|
| 2026-05-24 | **Dev Order 82 Constitutional Mode Transition complete** — Created CONSTITUTIONAL_CONTINUATION_NOTICE.md in tap-tone-pi establishing that the constitutional baseline (ADR-0009, ADR-0010, ADR-0011, ADR-0012, AGE_CONTRACT.md, ADVISORY_PRESENTATION_BOUNDARY.md) is sufficient for continued operational development. Ecosystem now operates in conditional constitutional stabilization mode: operational development is primary, constitutional escalation activates only when triggers occur (authority semantics ambiguity, epistemic status conflicts, boundary violations, provenance questions, sovereignty bypass, authority laundering). Prohibits speculative governance expansion, taxonomy refinement, and premature federation mechanics. Permits measurement workflows, advisory systems, epistemic status assignment, UI presentation, AGE development, export/import, archives, and topology experimentation within established boundaries. Key insight formalized: "Constitution cannot be completed in abstraction — it emerges through implementation, contradiction, and stabilization cycles." Operational development resumes as primary execution mode. Next focus: luthiers-toolbox Measurement Lab maturity. |
| 2026-05-24 | **Dev Order 81 Measurement Authority & Epistemic Status Constitutional Review complete** — Created constitutional doctrine for measurement authority and epistemic status taxonomy in tap-tone-pi. ADR-0011 (Measurement Authority Constitutional Definition) establishes: authoritative measurement artifact definition (captured, provenance-linked, unmodified, schema-compliant, instrument-class declared), measurement vs truth distinction, capture integrity requirements, contextual measurement legitimacy principle, provenance legitimacy, authority non-transfer rule, and authority laundering prevention with constitutional interruption points at each transition (measurement→advisory→historical→trust→canon). ADR-0012 (Epistemic Status Taxonomy) defines seven primary categories: OBSERVED (measurement-authoritative), DERIVED (computationally authoritative), ESTIMATED (approximation only), PREDICTED (model-dependent), HEURISTIC (no authority), OPERATOR-ANNOTATED (operator authority only), EXTERNALLY-SOURCED (external authority). Includes allowed/forbidden transitions, status inheritance rules (lowest authority wins), and display requirements. EPISTEMIC_STATUS_SCHEMA_IMPLICATIONS_REVIEW.md provides theoretical analysis of schema, export, UI, and propagation implications without committing to implementation. Key constitutional principles: measurement authority ≠ truth authority; measurement legitimacy is context-dependent; epistemic status cannot be silently elevated; operator sovereignty is final. NO runtime implementation, schema migration, or automation architecture introduced. |
| 2026-05-24 | **Dev Order 80 Advisory Presentation Patch Pass complete** — Applied four low-risk wording patches to tap-tone-pi advisory presentation surfaces. Path correction from handoff: actual repository locations are analyzer/reports/html_report.py and analyzer/guidance/engine.py (not tap_tone_pi/* paths). Patches applied: (1) html_report.py:353 Quality Grade → Quality Grade (heuristic), (2) html_report.py:426 Quality Grade → Quality Grade (heuristic), (3) html_report.py:430 Confidence → Advisory Confidence, (4) engine.py:168 Confidence: → Advisory confidence:. Verification: grep search confirms no remaining unqualified labels in audited surfaces. Updated ADVISORY_PRESENTATION_AUDIT.md to mark all four findings as remediated. No runtime, scoring, confidence calculation, advisory behavior, CI, or governance doctrine changed. Presentation-layer authority leakage gaps now closed. |
| 2026-05-23 | **Dev Order 79 Advisory Presentation & Confidence Semantics Boundary complete** — Hardened tap-tone-pi against presentation-layer authority leakage. Created ADVISORY_PRESENTATION_BOUNDARY.md defining visual/semantic rules for Measurement, Advisory, Interpretive, and Operator presentation domains. Added Section 7 (Presentation Boundary) to AGE_CONTRACT.md covering visual rules, confidence display rules, and text rules. Added Section 7 (Presentation-Layer Authority Leakage) to ADR-0010 covering risk categories and mitigation requirements. Completed full codebase audit documented in ADVISORY_PRESENTATION_AUDIT.md. Audit findings: strong structural compliance (all advisory modules have INSTRUMENT CLASS banners, WolfAdvisor has excellent boundary language, wood_properties has explicit heuristic annotation). Presentation gaps identified: html_report.py displays bare "Confidence" and "Quality Grade" without qualifiers; engine.py prompt includes bare "Confidence:". Four recommended patches deferred to owner review (not applied per threshold: changes affect visual/behavior, not obvious single-term replacements). Lexical CI guard documented as future hardening recommendation (not implemented per scope). No new advisory behavior, ranking, or runtime taxonomy changes introduced. |
| 2026-05-23 | **Dev Order 78 Analyzer Guidance Engine Constitutional Contract complete** — Created ADR-0010 (Advisory Authority Constitutional Boundary) and AGE_CONTRACT.md in tap-tone-pi repo. ADR-0010 establishes constitutional doctrine for all DECISION SUPPORT systems: orthogonal authority domains (Measurement, Advisory, Interpretive, Operator), operator sovereignty protection, forbidden vocabulary (confirmed, verified, diagnosed, resolved, proven, certified, validated, optimal, correct, approved, canonical, best, fixed), allowed vocabulary (observed, flagged, suggested for review, attention recommended, possible anomaly, potential drift). Runtime enforcement unchanged (MEASUREMENT vs DECISION SUPPORT classes preserved). AGE_CONTRACT.md operationalizes ADR-0010 for AnalyzerGuidanceEngine: read-only measurement access, AttentionDirectiveV1 output only, silent API fallback, thread isolation, stage-aware prompting, operator sovereignty preservation. Future governance hardening recommended (vocabulary auditing CI, UI authority linting) but not implemented. Key insight: tap-tone-pi uses structural governance while luthiers-toolbox uses lexical governance — combined they form constitutional semantics + runtime-enforced authority boundaries. Documents created at tap_tone_pi-main/docs/ADR-0010-advisory-authority-constitutional-boundary.md and tap_tone_pi-main/docs/AGE_CONTRACT.md |
| 2026-05-23 | **Dev Order 77 Tap-Tone-Pi Governance Consolidation Audit complete** — Audited existing tap-tone-pi governance structure (not creation of new governance). Key finding: mature CI-enforced measurement-vs-advisory separation via ADR-0009 and check_advisory_boundary.py. Identified consolidation opportunities: cross-referencing between GOVERNANCE.md and specialized ADRs, explicit AGE contract extraction from engine.py docstring, terminology alignment for "advisory" usages. Governance inventory: 4 core docs (GOVERNANCE.md, BOUNDARY_RULES.md, MEASUREMENT_BOUNDARY.md, BoundarySpec.md), 2 key ADRs (ADR-0004 acoustic-vs-structural, ADR-0009 advisory boundary), 5 agentic spine specs, 4 CI workflows. Recommendations: add ADR-0004 cross-ref to GOVERNANCE.md §9, add INSTRUMENT CLASS to promotion checklist, create GOVERNANCE_INDEX.md, consolidate AGE contract. No new governance requirements needed — existing structure is sound. Output: docs/handoffs/TAP_TONE_PI_GOVERNANCE_CONSOLIDATION_AUDIT.md |
| 2026-05-22 | **Dev Order 76 Measurement Lab Evidence Review Workspace complete** — Added observational evidence review layer for grouping, inspecting, flagging, and comparatively reviewing experimental records. EvidenceReviewRecord types with reviewState (reviewed/flagged/needs-followup/repeat-observed). Forbidden states explicitly rejected (approved/validated/canonical/certified). Utilities: createEvidenceReview, validateEvidenceReview, normalizeEvidenceReview, filterReviewsByTarget, filterReviewsByState, summarizeReviewsByTargetType, summarizeReviewsByState, dedupeEvidenceReviews. EvidenceReviewPanel with add form, state/target type filters, notes/tags, timestamps. Multiple reviews per target allowed (dedupe by reviewId only). Append-only, no editing/deletion. Neutral styling (no green/yellow/red). Notes linkage deferred. 68 tests pass covering all 15 required categories plus forbidden language audit. 341 total acoustics tests pass. Build verified. Boundary note: "Evidence reviews are observational workflow annotations and do not establish acoustic authority, validation, or canonical status." |
| 2026-05-22 | **Dev Order 75 Experiment Notes QA & Measurement Lab Notebook Hardening complete** — QA hardening pass for experiment notes layer. Added 7 new tests: createExperimentNote input immutability, filterNotesByTarget array immutability, summarizeExperimentNotes array immutability, dedupeExperimentNotes array immutability, forbidden language audit for buildExperimentNoteLabel, getTargetTypeLabel, and validateExperimentNote error messages. All 50 experiment note tests pass. 273 total acoustics tests pass. Build verified. Existing coverage confirmed: target type validation, target ID hardening, tag normalization. No new features — QA hardening only. |
| 2026-05-22 | **Dev Order 74 Measurement Lab Experiment Notes Layer complete** — Added local observational experiment notes for Measurement Lab records. Notes can attach to archives, topology variants, residual comparisons, correlations, drift records, and drift synthesis records. ExperimentNote types with targetType/targetId/text/tags. Utilities: createExperimentNote, validateExperimentNote, normalizeExperimentNote, filterNotesByTarget, summarizeExperimentNotes, dedupeExperimentNotes, buildExperimentNoteLabel. ExperimentNotesPanel with add form, filter by target type, timestamps, tags display, empty state with inline add affordance. Notes are in-memory only, append-only, no editing/deletion. Manual targetId entry only. Comma-separated tags with trim/dedupe. 43 tests pass. Boundary note: "Notes are local observational annotations and do not modify measurements or establish acoustic conclusions." No persistence, recommendation, calibration authority, or prediction introduced. |
| 2026-05-22 | **Dev Order 73 Drift Synthesis QA & Measurement Lab Boundary Hardening complete** — Added "causes", "proves" variants to forbidden word lists in drift and synthesis tests. Added test cases: variant grouping requires 3 records per variant (insufficientVariants), synthesis ID prefix format (drift-synthesis-YYYYMMDDHHMMSS), archive count deduplication. 41 synthesis tests pass, 50 drift tests pass, 223 total acoustics tests pass. Build verified. QA hardening only — no new features. |
| 2026-05-21 | **Dev Order 72 Experimental Drift Session Synthesis complete** — Added session-level drift synthesis infrastructure. Computes overall, per-session, and per-variant synthesis from drift records. New direction type "mixed" for conflicting directions. Generates prose pattern descriptions. Panel accepts archives, computes drift internally, then synthesizes. Boundary note: "Drift synthesis summarizes repeated observational patterns across experimental sequences and does not imply causation, optimization, or design superiority." 38 tests pass covering all 15 required categories. 220 total acoustics tests pass. |
| 2026-05-21 | **Dev Order 71 Drift Timeline QA & Observational Boundary Hardening complete** — Added variable direction detection (2+ sign reversals OR stdev >10%). Reused correlation validators (isValidArchiveForCorrelation, isSparseArchive, deduplicateArchives). computeExperimentalDrift now deduplicates and filters archives, returns warnings and totalArchivesProvided. Updated panel boundary notice to match spec. Added 18 new tests covering validation, filtering, variable detection, input mutation. 50 tests pass. |
| 2026-05-21 | **Dev Order 70 Experimental Drift Timeline Workspace complete** — ExperimentalDriftTimelinePanel for temporal drift observations across sequential measurement archives. Tracks drift direction (upward/downward/stable/variable) using 5% threshold. Groups by context (chronological/variant/session). Minimum 3 archives for drift detection. Timeline points with deltaFromPrevious and percentDeltaFromPrevious. Summaries by context with dominant direction. Forbidden language enforcement matches correlation semantics. 22 tests pass including observational language audit. |
| 2026-05-21 | **Dev Order 69 Experimental Correlation QA & Boundary Hardening complete** — Fixed confidence band threshold (6+ for emerging, was incorrectly 5+). Added archive validation (isValidArchiveForCorrelation), sparse archive detection (isSparseArchive), filterCorrelatableArchives, deduplicateArchives utilities. computeCorrelations and hasSufficientDataForCorrelations now deduplicate and filter invalid archives. Updated panel boundary notice to match spec. Added 16 new test cases covering validation, deduplication, input mutation, no-causality-fields assertions. 39 tests pass. No new features — QA hardening only. |
| 2026-05-21 | **Dev Order 68 Experimental Correlation Workspace complete** — ExperimentalCorrelationPanel for computed observational correlations between topology variants and residual behavior. Confidence bands (low/limited/emerging) based on sample count. Minimum 3 archives per variant. Forbidden language enforcement: no causation, optimization, recommendation semantics. Allowed language: correlates with, observed alongside, appears with. 23 tests pass. |
| 2026-05-20 | **Dev Order 67 Topology Variant QA & Measurement Lab Linkage Hardening complete** — Added groupArchivesByTopologyVariant, archive-variant linkage utilities for correlation computation. Supports Dev Order 68 infrastructure. |
| 2026-05-20 | **Dev Order 66 Topology Variant Framework complete** — TopologyVariant type and utilities for experimental acoustic configuration descriptors. TopologyVariantCard and TopologyVariantBuilder components. 72 tests pass. |
| 2026-05-20 | **Dev Order 65 Measurement Archive Evidence Index Views complete** — Chronological, by-method, and by-experiment views in MeasurementArchiveEvidenceIndex. Sparse archive detection and experiment tag grouping. |
| 2026-05-20 | **Dev Order 64 Measurement Lab Workflow QA & UX Stabilization complete** — Verified archive creation/import/export, evidence indexing, residual comparison, sparse-state handling, and observational language boundaries. Added duplicate detection, safe timestamp parsing, merge utilities. 58 tests pass. No persistence, calibration authority, prediction, ranking, or recommendation behavior introduced. |
...
| 2026-05-20 | **Dev Order 61 Measurement Archive Hardening complete** — Timezone fix (UTC methods in filename generation), test coverage expanded, boundary discipline verification. |
| 2026-05-20 | **Dev Order 60 Measurement Archive Integration complete** — MeasurementArchiveExchangeSection with export/import/preview cards mounted in Measurement Lab tab. Schema: measurement-archive.v1. Local-only, observational-only semantics. |
...
| 2026-05-14 | **Dev Order 49 Snapshot Exchange State Density Refinement complete** |
| 2026-05-14 | **Dev Order 48 Snapshot Exchange Micro-Interaction Consistency complete** |
