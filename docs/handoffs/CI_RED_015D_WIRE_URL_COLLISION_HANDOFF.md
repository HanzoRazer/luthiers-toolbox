...
> **Update 2026-05-30 (second defect found + fixed):** The `wire_url_audit.json` that produced the tables below was generated with a **second** bug: `load_manifest_entries()` in `scripts/audit_wire_urls.py` used a single combined regex whose lazy `[^)]*?` separators skipped the optional `prefix` group, so **every manifest entry was recorded with `manifest_prefix=""`** — the manifest prefix was dropped entirely. Evidence: the JSON shows `app.rmos.runs_v2.api_runs` with `manifest_prefix:""` and `wire_url:"/runs"`, but `rmos_manifest.py` mounts it at `/api/rmos` (real URL `/api/rmos/runs`). Its apparent `GET /runs` collision partner `acoustics_router` mounts at `/api/rmos/acoustics` (`/api/rmos/acoustics/runs`) — **different URLs, false positive.** The regex is now fixed (per-field parse), but `wire_url_audit.json` is **stale** until the script is re-run (`cd services/api && python scripts/audit_wire_urls.py`). **Net:** between this prefix bug and the unresolved composition (above), the "68 collisions" figure should not be cited as a real count. The MVP-path static trace (2026-05-30) found **no MVP-blocking collision**; see `SPRINTS.md → CI-RED-015-D`.
