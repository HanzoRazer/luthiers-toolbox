...
| 9 | **BR-044** — frontend radiation-ratio scale collapses rating and colour discrimination | CONFIRMED_DEFECT | small–moderate. **READY FOR BOUNDED DEV ORDER — NOT AUTHORIZED.** Proof on `br-044a`: reproduction + component witness + TC-12 adapter finding + TC-14 inventory + authority analysis. Scope: one local producer correction (`×1000` → `c/ρ`); three label/prose surfaces; a rounding-policy decision; test inversion; sort-invariance regression. **No API/type migration** — backend transport deferred |
