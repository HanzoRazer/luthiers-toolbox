...
| BR-002 | `list_runs_filtered()` rejects `tool_kind` (TypeError) | rmos/runs_v2 | **actual path** `app/saw_lab/executions_list_service.py:62` → `app.rmos.runs_v2.store.list_runs_filtered(tool_kind=…)` (prior citation `store_api.py:200` was **wrong**); shared filter `store_filter.matches_index_meta` has no `tool_kind`; `tests/test_saw_lab_endpoint_smoke.py:24` xfail | A | CONFIRMED_DEFECT | code-inspection + test-encoded | high | **SCOPE CORRECTION REQUIRED** | central `runs_v2` store + shared filter (broad blast radius) — resolve via **BR-002A** (archaeology) → **BR-002B** (repair) |
