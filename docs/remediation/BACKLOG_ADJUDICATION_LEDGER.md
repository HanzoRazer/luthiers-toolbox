...
| BR-021 | CI gates suppressed: `client_lint_build` continue-on-error (TS errors); vue_decomposition non-blocking | ci/client | `.github/workflows/client_lint_build.yml:42`, `vue_decomposition_gate.yml:79` | A | MAINTAINABILITY_DEBT | code-inspection | med | **partial — type-check ratcheted** | **type-check lane RESOLVED-BY-RATCHET** (`ci/type_error_gate.py` + `ci/type_error_baseline.json`): `continue-on-error` removed, error count **frozen at 150** (measured; the "400+" comment was stale), gate **fails on any increase** so a TS-major bump can no longer pass green. Remaining under BR-021: the **eslint** lane (`:46`, same shape, partial `--max-warnings=1200`) and **`vue_decomposition_gate.yml:79`** — same ratchet pattern applies. |
