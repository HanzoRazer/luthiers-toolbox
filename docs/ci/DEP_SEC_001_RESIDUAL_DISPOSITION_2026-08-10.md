...
|----|--------|------------|-------------|-----------------|
| R-01 | INHERITED | Archive Dependabot alerts (9 × vite in `archive/**`, `docs/archive/**`) | **OWNER ACTION** | Dismiss in GitHub UI as unused; template in Tier-1 closeout §5. Agent API 403. |
| R-02 | INHERITED | Alert ledger recalculation witness after #253 + archive dismissals | **OWNER ACTION** / **BLOCKED** (on API) | Owner UI witness; do not invent counts. Recalc is expected *after* Tier-1 patches — not evidence Tier-1 failed. |
...
| R-07 | INHERITED | Remaining #252 triage packages after axios/postcss (toolchain/transitives as still alerted) | **DEFERRED** → **Tranche B** (security residual) after owner alert witness | Snapshot-dependent; re-ground against live alerts before implementing |
| R-08 | FAN-OUT | Version-hygiene majors/patches from #254–#258 | **DEFERRED** → **Tranche B** | Not #252 security packages; authorize explicitly before merge |
...
| R-10 | INHERITED | Residual `npm audit` surface in `packages/client` after Tier-1 patch (**21** issues at closeout witness) | **DEFERRED** → **Tranche B** / **Tranche C** as classified after alert witness | Documented in Tier-1 closeout §6; **not** a Tier-1 failure criterion; largely toolchain overlap with R-04/R-05/R-07 |
