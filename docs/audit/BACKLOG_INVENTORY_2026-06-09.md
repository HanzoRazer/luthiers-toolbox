...
| 9 | Consolidator B-sanctions (`dxf_consolidator` dead code; `layer_consolidator` internal-temp) | audit 2026-06-09 | **DONE (dxf)** | STALE-FIX (doc) | POST-MVP | dxf=no callers (2nd-pass empty); layer=internal IBG temp | Sanction docs + dxf deprecation flag → **EXECUTED**: `dxf_consolidator` RETIRED (deprecation flag + live matrices reconciled). `layer_consolidator` reclassified PENDING-CANONICALIZATION (route through DxfWriter on completion). |
