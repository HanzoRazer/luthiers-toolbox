...
| `PROFILING-ROUTE-ANNOTATION-001` — `@safety_critical` + postponed annotations + bare Pydantic body → FastAPI 422 `loc=['query','req']` before the handler runs. Same pattern on profiling, binding, V-carve production, inlay export, radius-dish generate, and feeds-speeds. | **FIXED at the decorator** — see `docs/audit/PROFILING-ROUTE-ANNOTATION-001.md`. Restores FastAPI body binding; does **not** add RMOS evaluators. 001B-B2 HOLD on *gating* those siblings is unchanged. |
