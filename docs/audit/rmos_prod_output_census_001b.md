...
| `Governance Baseline Diff Report` (`baseline-diff`) computes its report correctly but **403s posting it** — the workflow declares no `permissions:` block, so the default token cannot `issues.createComment`. Its comment step is gated on the `governance` label, so the step had never executed until this PR; it broke on first use. Not a required check | **Queue — CI infrastructure**; non-blocking (`Fence Checks (Blocking)` and `CBSP21 Patch Manifest Gate` are the only required contexts) |
