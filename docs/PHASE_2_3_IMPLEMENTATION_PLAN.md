...
### 2026-03-07: Stub Debt Remediation (Phase 2.1)

**Session Focus:** Wire stub endpoints to real implementations

#### Completed

| Endpoint | Proxied To | Tests | Commit |
|----------|------------|-------|--------|
| `/api/cam/posts`, `/posts/{id}` | `posts_consolidated_router` | 4 | `ae9635dd` |
| `/api/cam/logs/write` | `logs_router` | 4 | `740238b2` |
| `/api/cam/job-int/log` (3 endpoints) | `job_int_log` service | 6 | `d044f5f7` |
| `/api/cam/fret_slots/preview` | `fret_slots_cam` calculator | 7 | `a309939c` |
| `/api/cam/probe/svg_setup_sheet` | `setup_router` | — | `5d557a38` |
| `/api/cam/adaptive2/*` (3 endpoints) | `benchmark_router` | — | `36e38127` |

**Totals:** 11 endpoints proxied, 21 smoke tests added

#### Previously Removed (Real Implementations Exist)

| Former Stub | Real Implementation |
|-------------|---------------------|
| `/backup/*` | `cam/routers/utility/backup_router.py` |
| `/drilling/*` | `cam/routers/drilling/drill_router.py` |
| `/pocket/adaptive/*` | `routers/adaptive/*.py` |
| `/blueprint/preflight,to-adaptive,reconstruct` | `routers/blueprint_cam/*.py` |
| `/opt/what_if,feeds-speeds` | `cam/routers/utility/optimization_router.py` |
| 20 dead code stubs | Removed in cleanup commits |

#### Remaining Stubs (23 total)

| File | Count | Priority |
|------|-------|----------|
| `cam/routers/stub_routes.py` | 4 | LOW |
| `rmos/stub_routes.py` | 13 | HIGH (analytics, rosette CNC) |
| `routers/misc_stub_routes.py` | 6 | MEDIUM (AI advisories, blueprint) |

#### Stub Debt Metrics

| Metric | Original (Feb 23) | Current | Change |
|--------|-------------------|---------|--------|
| Total Stubs | 73 | 23 | **-68%** |
| Proxied (Functional) | 0 | 11 | +11 |
| Removed Entirely | 0 | ~39 | +39 |
| Smoke Tests | 0 | 21 | +21 |
...
4. [x] Begin Week 1: Missing routes implementation *(stub remediation in progress)*
5. [ ] Create sprint board with all tasks
6. [ ] Wire `/rmos/analytics/*` stubs to run store aggregation
7. [ ] Wire `/rmos/rosette/*` stubs to art_studio geometry engine
8. [ ] Wire `/ai/advisories/request` to AI transport layer

---

*Generated by Claude Code analysis of luthiers-toolbox codebase and website structure.*
*Progress log maintained during implementation.*