...
|-----|---------------|-------------|
| ~~LP-GAP-03~~ | `app/cam/neck/` | ~~Neck CNC pipeline missing~~ **RESOLVED** (modular pipeline with full-scale station awareness) |
| ~~BEN-GAP-08~~ | `app/cam/carving/` | ~~3D surface carving (archtop)~~ **RESOLVED** (parametric ellipsoidal dome, graduation maps, roughing + finishing) |
...
3. **`app/cam/neck/` module** — ✅ **RESOLVED** (2026-03-15) Neck CNC pipeline: truss rod channel, profile carving (with station awareness beyond 12"), fret slot integration. 38 tests. Resolves LP-GAP-03.
...
5. **`app/cam/carving/` module** — ✅ **RESOLVED** (2026-03-15) 3D surface carving: parametric ellipsoidal dome with Benedetto recurve, graduation maps with bilinear interpolation, parallel-plane roughing, raster finishing with scallop-controlled stepover. 33 tests. Resolves BEN-GAP-08.
