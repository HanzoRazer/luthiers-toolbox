...
import NeckView          from '@/views/NeckView.vue'
import ConfiguratorView    from '@/views/ConfiguratorView.vue'
import CamWorkspaceView   from '@/views/CamWorkspaceView.vue'

const activeTab = ref<'workspace' | 'import' | 'parametric' | 'neck' | 'configure' | 'cam'>('workspace')
...
<template>
  <div class="shell">
    <nav class="shell-nav">
      ...
      <div class="nav-tabs">
        <button v-for="tab in ['workspace','import','parametric','neck','configure','cam'] as const" :key="tab"
          class="nav-tab" :class="{ on: activeTab === tab }" @click="activeTab = tab">
          <span class="tab-dot"></span>
          {{ tab.charAt(0).toUpperCase() + tab.slice(1) }}
          <span v-if="tab === 'parametric'" class="pro-badge">Pro</span>
          <span v-if="tab === 'configure'" class="pro-badge" style="background:rgba(90,184,106,.1);border-color:#3a8850;color:#5ab86a">Customer</span>
          <span v-if="tab === 'cam'" class="pro-badge" style="background:rgba(91,143,168,.12);border-color:#5b8fa8;color:#8ab8d0">CNC</span>
        </button>
      </div>
    </nav>
    <main class="shell-main">
      ...
      <NeckView          v-show="activeTab === 'neck'"       @toast="showToast" />
      <ConfiguratorView  v-show="activeTab === 'configure'"   @toast="showToast" />
      <CamWorkspaceView  v-show="activeTab === 'cam'"           @toast="showToast" />
    </main>
