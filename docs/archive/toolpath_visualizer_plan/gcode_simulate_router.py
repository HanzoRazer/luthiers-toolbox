...
Shares the /api/cam/gcode prefix with gcode_backplot_router.py.
...
router = APIRouter(prefix="/api/cam/gcode", tags=["cam", "gcode", "simulate"])
...
def simulate_gcode(req: SimulateRequest) -> SimulateResponse:
    ...
    Args:
        req: SimulateRequest with gcode, unit mode, feed rates, arc resolution

    Returns:
        SimulateResponse with segments[], bounds, and aggregate totals
