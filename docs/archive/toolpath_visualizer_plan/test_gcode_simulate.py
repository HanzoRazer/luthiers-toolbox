...
# ---------------------------------------------------------------------------
# Import helpers — isolate from the FastAPI dependency chain
# ---------------------------------------------------------------------------
...
simulate_segments = _mod.simulate_segments
...
def test_arc_ends_at_correct_position():
    ...
    # Half-circle: start (10, 0), center (5, 0) via I=-5 J=0, end (0, 0)
...
def test_full_circle_ij_produces_segments():
    # G2 where start and end are the same → full circle
...
def test_mixed_program_order_preserved():
    """First segment must be a rapid (G0 X0 Y0 → position change from Z only? No — no XY change.
    Use a program where first command is definitely a rapid XY move."""
    gcode = "G0 X10 Y0\nG1 X20 F600\nG2 X30 Y10 I5 J5"
    r = run(gcode)
...
def test_line_numbers_are_1_based_and_sequential():
    gcode = "G0 X10\nG1 X20 F600\nG0 X0"
    r = run(gcode)
    # Each segment must reference a valid, positive line number
    for seg in r["segments"]:
...
def test_line_text_matches_source():
    gcode = "G0 X10 Y0\nG1 X20 F600"
    r = run(gcode)
    # Rapid segment should reference the G0 line
...
def test_duration_parity_with_simulate(gcode):
    ...
    """
    old = simulate(gcode)
    new = run(gcode, arc_resolution_deg=1)  # high resolution for accuracy
