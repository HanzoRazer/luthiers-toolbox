...
def generate_file_list_json(files: List[Dict], output_path: Path):
    ...
    print(f"Γ£à Generated: {output_path}")

def main():
    """Main entry point."""
    root = Path.cwd()
    print(f"≡ƒöì Scanning: {root}")
    
    # Scan for files
    files = scan_directory(root, recursive=False)
    
    print(f"\n≡ƒôü Found {len(files)} files:")
    for f in files:
        print(f"  ΓÇó {f['name']} ({f['type'].upper()}) - {f['size']:,} bytes")
    
    # Generate JSON file list
    output_json = root / 'files.json'
    generate_file_list_json(files, output_json)
    
    print(f"\nΓ£à Done! The index.html will auto-load from files.json")
    print(f"≡ƒÆí Tip: Run this script whenever you add new files")
