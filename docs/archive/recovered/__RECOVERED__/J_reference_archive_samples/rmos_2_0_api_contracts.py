1∩╕ÅΓâú RMOS 2.0 API Factory (api_contracts.py)
This is a full replacement for the earlier api_contracts.py we drafted. It:
ΓÇó	Keeps the same public models (RmosContext, RmosFeasibilityResult, etc.).
ΓÇó	Adds an RmosServices dataclass as the central wiring point.
ΓÇó	Adds get_rmos_services() which acts as the factory for the active RMOS implementation.
ΓÇó	Makes the public functions (compute_feasibility_for_design, etc.) just thin delegators to the services instance.
"""
...
class RmosFeasibilityResult(BaseModel):
    ...
    """
    overall_score: float                 # 0ΓÇô100
    risk_bucket: RiskBucket              # GREEN / YELLOW / RED
    material_efficiency: float           # 0ΓÇô1
...
ΓÇªand youΓÇÖve got a single, controlled place (get_rmos_services) where the implementations are chosen.
