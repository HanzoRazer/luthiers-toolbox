Absolutely. Below is a **direct mapping** of the five repo-wide requirement docs onto the **workflow state machine** I drafted earlier, plus a **drop-in updated `state_machine.py`** that bakes those requirements into the workflow contract.

The goal is simple:

> **Every feasibility/toolpath action produces a Run Artifact.
> Toolpaths always trigger server-side feasibility recompute.
> The UI + Diff viewer become “views” over these artifacts.**

---

# 1) Requirements → State Machine Mapping

## A) SERVER_SIDE_FEASIBILITY_ENFORCEMENT.md → State Machine

**Requirement:** Toolpaths must not trust client feasibility; must recompute server-side.

**State machine changes:**

* `request_toolpaths()` must **force a server feasibility recompute** step before allowing toolpaths.
* Add a flag and event metadata: `feasibility_source="server_recompute"`.

**Implementation:**

* Introduce a dedicated action/state pair:

  * `TOOLPATHS_REQUESTED → FEASIBILITY_REQUESTED (recompute)` or a cleaner approach:
  * Keep states but require `session.meta["server_feasibility_hash"]` set before toolpaths.

I’m taking the cleanest, auditable approach:

✅ **Add a “server-side feasibility required” guard in `request_toolpaths()`**
✅ **Track feasibility hash/version in `FeasibilityResult.meta`**
✅ **Store “toolpaths request” as an artifact even when blocked**

---

## B) RUN_ARTIFACT_PERSISTENCE.md → State Machine

**Requirement:** Persist run artifacts for feasibility and toolpaths calls (OK/BLOCKED/ERROR).

**State machine changes:**

* Every action that can block must create a `RunArtifactRef` (even if blocked).
* Store artifact references on the session:

  * `session.last_feasibility_artifact`
  * `session.last_toolpaths_artifact`
* Emit events containing `artifact_id`.

✅ This is what makes auditing, UI panels, and diff viewer possible.

---

## C) RUN_ARTIFACT_INDEX_QUERY_API.md → State Machine

**Requirement:** Artifacts must be indexable/queryable by mode, tool_id, risk, status, timestamps.

**State machine changes:**

* Ensure session meta captures:

  * `tool_id`, `material_id`, `machine_id`
  * `mode` (design-first / constraint-first / ai-assisted)
  * `status` (OK/BLOCKED/ERROR)
  * `risk_bucket`, `score`
  * `design_hash`, `context_hash`, `policy_version`, `calculator_versions`

✅ The state machine will require these “index fields” be present in the artifact meta.

---

## D) RUN_ARTIFACT_UI_PANEL.md → State Machine

**Requirement:** UI needs predictable artifact links and summaries.

**State machine changes:**

* Provide stable accessors:

  * `get_last_artifact_refs()`
  * `get_ui_summary(session)`
* Ensure every terminal state can surface:

  * last feasibility artifact
  * last toolpaths artifact
  * last decision + warnings

✅ UI panel becomes a read-only view over state machine output + artifacts.

---

## E) RUN_DIFF_VIEWER.md → State Machine

**Requirement:** Must diff runs based on artifact hashes, feasibility payloads, policy versions, and toolpath plans.

**State machine changes:**

* Store `design_hash`, `context_hash`, `toolpath_hash`, `feasibility_hash`.
* Store `policy_version` and `calculator_version_map`.
* Expose a `RunComparableSnapshot` structure so the diff viewer can do a stable diff without reading raw logs.

✅ Diff viewer becomes deterministic: compare two artifacts or two session snapshots.

---

# 2) Drop-in Updated State Machine File

Save this as:

`services/api/app/workflow/state_machine.py`

This version is still “thin” (does not import your RMOS module), but it is now explicitly **artifact-driven** and **server-feasibility enforced**.
...
# 3) What You Still Need to Wire in the Routers/Services

This file deliberately **does not** write artifacts to disk/db. That’s correct separation of concerns.

But to fully satisfy the repo-wide docs, your feasibility/toolpaths endpoints must do this:

### Feasibility endpoint flow (server)

1. `request_feasibility(session)`
2. Compute feasibility (server)
3. Persist run artifact (`kind="feasibility"`) with:

   * inputs hashes, calculator versions, policy version, warnings, risk bucket
4. `attach_feasibility_artifact(session, artifact_ref)`
5. `store_feasibility(session, result)`

### Toolpaths endpoint flow (server)

1. **Always recompute feasibility** (even if provided)
2. Persist feasibility artifact (again) with `source="server_recompute"`
3. Store feasibility on session
4. `approve()` if your workflow requires explicit approval
5. `request_toolpaths()` (will now enforce server feasibility source)
6. Generate toolpaths
7. Persist toolpaths artifact (`kind="toolpaths"`)
8. `attach_toolpaths_artifact(session, artifact_ref)`
9. `store_toolpaths(session, plan_ref)`

This satisfies:

* Server-side feasibility enforcement
* Artifact persistence
* Index/query requirements (via metadata)
* UI panel (artifact refs)
* Diff viewer (comparable snapshot + hashes)

---

# 4) Two Tight Questions (Needed to Finalize Enforcement)

1. **Should `require_server_side_feasibility_for_toolpaths` be hardcoded True in production builds?**
   If yes, we should remove the toggle (or lock behind env var), so it can’t be disabled accidentally.

2. **Do you want artifacts created even when an exception occurs (ERROR status)?**
   The requirements imply yes. That means your router should “catch, persist ERROR artifact, then re-raise”.

---

If you want, next I can generate the **exact FastAPI wrapper code** for:

* `/api/rmos/feasibility`
* `/api/rmos/toolpaths`

that:

* calls this state machine
* recomputes feasibility server-side
* persists artifacts (OK/BLOCKED/ERROR)
* returns `artifact_id` for the UI panel immediately
