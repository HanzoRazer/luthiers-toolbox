...
1. Missing context values ΓåÆ token remains literal (e.g., "{neck_profile}" if neck data unavailable)
2. Empty/None values ΓåÆ token removed from output
3. Tokens are case-insensitive ({Preset} = {preset})
4. Unknown tokens ΓåÆ remain literal for forward compatibility
5. Special characters in values ΓåÆ sanitized (spaces ΓåÆ underscores, slashes removed)
...
def sanitize_filename_part(value: str) -> str:
    ...
    Rules:
    - Spaces ΓåÆ underscores
    - Forward/backslashes ΓåÆ removed
    - Colons ΓåÆ hyphens (for timestamps)
    - Multiple underscores ΓåÆ single underscore
    - Leading/trailing underscores ΓåÆ trimmed
...
def resolve_template(template: str, context: Dict[str, Any]) -> str:
    ...
    def replace_token(match: re.Match) -> str:
        ...
        # Unknown token ΓåÆ keep literal for forward compatibility
        return match.group(0)
    
    # Replace all tokens
    resolved = re.sub(token_pattern, replace_token, template)
    
    # Clean up multiple consecutive delimiters (e.g., "___" ΓåÆ "__")
