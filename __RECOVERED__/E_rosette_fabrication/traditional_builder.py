...
class CutList:
    ...
    def __str__(self) -> str:
        return (
            ...
            f"{self.num_strips:3}├ù strips @ "
            f"{self.strip_width_mm:.1f}mm wide ├ù "
...
class StickDefinition:
    ...
    def __str__(self) -> str:
        parts = [f"{count}├ù {species}" for species, count in self.strips]
...
class TraditionalBuilder:
    ...
    def create_project(
        ...
        for stick in stick_defs:
            strip_desc = ", ".join(f"{count}├ù {species}" for species, count in stick.strips)
...
def demo():
    ...
    for master, patterns in builder.list_master_patterns().items():
        ...
        for p in patterns[:3]:  # Show first 3
            ...
            print(f"    ΓÇó {info['name']} ({info['rows']}├ù{info['columns']})")
