...
# ============================================================================
print("\n1. Creating synthetic DXF with 100x60mm rectangle...")
...
try:
    ...
    # Create closed rectangle (100x60 mm)
    points = [
        (0, 0),
        (100, 0),
        (100, 60),
    ...
    print(f"  [OK] DXF created: {len(dxf_data)} bytes")
    print(f"  [OK] Rectangle: 100x60 mm on GEOMETRY layer")

except Exception as e:
    print(f"  [FAIL] Error creating DXF: {e}")
    sys.exit(1)
...
try:
    ...
    print(f"  [OK] Loops extracted: {len(loops)}")
    ...
    if len(loops) == 0:
        print(f"  [FAIL] Expected 1 loop, got {len(loops)}")
        sys.exit(1)
    
    # Validate loop structure
    loop = loops[0]
    print(f"  [OK] Loop has {len(loop.pts)} points")
    
    if len(loop.pts) != 4:
        print(f"  [FAIL] Expected 4 points, got {len(loop.pts)}")
        sys.exit(1)
    ...
    for i, (expected, actual) in enumerate(zip(expected_points, loop.pts)):
        if abs(expected[0] - actual[0]) > 0.01 or abs(expected[1] - actual[1]) > 0.01:
            print(f"  [FAIL] Point {i} mismatch: expected {expected}, got {actual}")
            sys.exit(1)
    
    print(f"  [OK] All points match expected coordinates")
...
try:
    ...
    print(f"  [OK] Bridge endpoint successful")
    print(f"  [OK] Loops extracted: {result['loops_extracted']}")
    print(f"  [OK] Moves generated: {result['stats']['move_count']}")
    print(f"  [OK] Cutting moves: {result['stats']['cutting_moves']}")
    print(f"  [OK] Toolpath length: {result['stats']['length_mm']} mm")
    print(f"  [OK] Estimated time: {result['stats']['time_min']} min")
    print(f"  [OK] Volume removed: {result['stats']['volume_mm3']} mm³")
    
    # Validate moves structure
    moves = result['moves']
    if len(moves) < 5:
        print(f"  [FAIL] Too few moves: {len(moves)} (expected >5)")
        sys.exit(1)
    ...
    if not has_g0:
        print(f"  [FAIL] No G0 (rapid) moves found")
        sys.exit(1)
    
    if not has_g1:
        print(f"  [FAIL] No G1 (feed) moves found")
        sys.exit(1)
    
    print(f"  [OK] Move types validated (G0 rapids + G1 feeds)")
    
    # Validate statistics
    stats = result['stats']
    if stats['length_mm'] <= 0:
        print(f"  [FAIL] Invalid toolpath length: {stats['length_mm']}")
        sys.exit(1)
    
    if stats['time_s'] <= 0:
        print(f"  [FAIL] Invalid time estimate: {stats['time_s']}")
        sys.exit(1)
    
    print(f"  [OK] Statistics validated")
...
try:
    ...
    print(f"  [OK] DXF with island created")
    
    # Test extraction
    loops_island, warnings_island = extract_loops_from_dxf(dxf_data_island, "GEOMETRY")
    
    if len(loops_island) != 2:
        print(f"  [FAIL] Expected 2 loops (outer + island), got {len(loops_island)}")
        sys.exit(1)
    
    print(f"  [OK] Extracted {len(loops_island)} loops (outer + island)")
    ...
    if response_island.status_code != 200:
        print(f"  [FAIL] HTTP {response_island.status_code}: {response_island.text}")
        sys.exit(1)
    
    result_island = response_island.json()
    
    print(f"  [OK] Bridge with island successful")
    print(f"  [OK] Loops: {result_island['loops_extracted']}")
    print(f"  [OK] Moves: {result_island['stats']['move_count']}")
    print(f"  [OK] Length: {result_island['stats']['length_mm']} mm")
    ...
    if result_island['stats']['move_count'] <= result['stats']['move_count']:
        ...
        print(f"  [OK] Island avoidance working (more moves with island)")
...
try:
    ...
    print(f"  [OK] Bridge: {health.get('bridge')}")
    print(f"  [OK] Endpoints: {health.get('endpoints')}")
    ...
    if integration.get('adaptive_l1') != 'available':
        ...
        print(f"  [OK] Adaptive L.1 integration available")
