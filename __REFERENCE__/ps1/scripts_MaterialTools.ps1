Updated scripts/MaterialTools.ps1
<#
.SYNOPSIS
...
How to use the round-trip validator

From your repo root in PowerShell:

.\scripts\MaterialTools.ps1 materials-roundtrip-test


What it does:

Reads tests/data/saw_lab_materials.csv

Runs materials_csv_to_json.py → _materials_roundtrip.json

Runs materials_json_to_csv.py → _materials_roundtrip.csv

Compares _materials_roundtrip.csv to the original CSV

Prints GREEN success if identical, otherwise YELLOW/RED warning and exits with code 1

This is also CI-friendly: you can add this as a job step, and your pipeline will fail if a change to the converters stops being lossless over a CSV→JSON→CSV cycle.