  ...
  <div class="border-l first:border-l-0 flex flex-col">
    <div class="px-3 py-2 text-sm font-semibold bg-slate-50 flex items-center gap-2">
      ...
      <div class="flex items-center gap-2">
        ...
        <button
          class="px-2 py-1 text-xs border rounded bg-green-50"
      ...
      <table class="w-full">
        <tbody>
          <tr
            v-for="(ln,i) in lines"
            :key="i"
            :class="inHint(i) ? 'bg-yellow-100' : ''"
          >
            <td class="px-2 py-0.5 text-right text-gray-400 w-10 select-none">
              {{ i+1 }}
            </td>
            <td
              class="px-2 py-0.5 whitespace-pre break-words"
              v-html="esc(ln)"
            />
          </tr>
        </tbody>
