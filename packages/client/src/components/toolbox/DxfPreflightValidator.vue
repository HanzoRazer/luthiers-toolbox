...
async function validateFile() {
  ...
  try {
    ...
    const response = await fetch('/dxf/preflight/validate', {
      method: 'POST',
      body: formData
    })
...
async function applyAutoFix() {
  ...
  try {
    ...
    const response = await fetch('/dxf/preflight/auto_fix', {
      method: 'POST',
...
async function validateFixedDxf() {
  ...
  try {
    const response = await fetch('/dxf/preflight/validate_base64', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
