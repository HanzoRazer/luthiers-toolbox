<template>
  ...
  >
    ...
    <section class="border rounded p-3 bg-white space-y-1">
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-2">
        <div>
          ...
          <div class="text-sm font-semibold">
            {{ analytics.global_summary.total_jobs }}
          </div>
        </div>
        <div>
          <div class="text-[11px] text-gray-500">
          ...
          <div class="text-sm font-semibold">
            {{ analytics.global_summary.total_presets }}
          </div>
        </div>
        <div>
          <div class="text-[11px] text-gray-500">
        ...
        <div>
          ...
          <div
            ...
            :class="fragilityColorClass(analytics.global_summary.overall_fragility_score)"
          >
      ...
      <div class="flex gap-2 pt-2">
        <div
          v-for="(count, grade) in analytics.global_summary.grade_counts"
          :key="grade" 
          ...
          <tbody>
            ...
            >
              ...
              <td
                ...
                :class="fragilityColorClass(m.avg_fragility)"
              >
                {{ formatNumber(m.avg_fragility) }}
              </td>
              <td
                class="p-1 text-right font-semibold"
                :class="fragilityColorClass(m.worst_fragility)"
              >
        ...
        <table class="w-full border-collapse text-[11px]">
          ...
          <tbody>
            ...
            >
              ...
              <td class="p-1 text-right">
                {{ formatNumber(lane.avg_risk_score) }}
              </td>
              <td
                class="p-1 text-right"
                :class="fragilityColorClass(lane.avg_fragility_score)"
              >
                {{ formatNumber(lane.avg_fragility_score) }}
              </td>
              <td class="p-1">
                <span
                  v-if="lane.dominant_material_types.length"
    ...
    <!-- LANE TRANSITIONS -->
    <section
      v-if="analytics.lane_transitions.length > 0"
        ...
        <table class="w-full border-collapse text-[11px]">
          ...
          <tbody>
            <tr
              ...
              class="border-b last:border-0 hover:bg-gray-50"
            >
              <td class="p-1 font-mono truncate max-w-[120px]">
                {{ formatTimestamp(r.created_at) }}
              </td>
              <td class="p-1 font-mono text-[10px]">
                {{ r.preset_id || '—' }}
              </td>
              <td class="p-1 font-mono">
                {{ r.lane }}
              ...
              <td class="p-1">
                <span
                  class="px-1 rounded text-[9px] font-mono"
                  :class="gradeColorClass(r.risk_grade)"
                >
                  {{ r.risk_grade }}
                </span>
              </td>
              <td
                class="p-1 text-right"
                :class="fragilityColorClass(r.worst_fragility_score)"
              >
  ...
  <div
    v-else-if="error"
    class="p-4 text-xs text-red-600"
  >
