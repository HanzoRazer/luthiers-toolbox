 ...
 *
 * Displays workflow state transition history.
 * Shows last 5 events in reverse chronological order.
 */
...
const props = defineProps<{
  history: WorkflowEvent[] | null | undefined
}>()

const displayedHistory = computed(() => {
  if (!props.history?.length) return []
  return props.history.slice().reverse().slice(0, 5)
})
</script>
