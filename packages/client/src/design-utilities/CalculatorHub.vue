<template>
  <div class="calculator-hub">
    ...
    >
      ...
      <!-- Lutherie Geometry Calculators -->
      <div class="calc-category">
        <h3>🎸 Lutherie Geometry</h3>
        <div class="calc-cards">
          <div
            class="calc-card active"
            @click="selectCalculator('string-tension')"
          >
            <div class="calc-icon">
              🎵
            </div>
            <h4>String Tension</h4>
            <p>T = UW × (2LF)² / 386.4 · neck load · top load · break angle</p>
            <span class="status-badge ready">✓ Ready</span>
          </div>
          <div
            class="calc-card placeholder"
            @click="selectCalculator('fret-spacing')"
          >
            <div class="calc-icon">
              📏
            </div>
            <h4>Fret Spacing</h4>
            <p>Scale length → fret positions, inter-fret spacings</p>
            <span class="status-badge">Coming Soon</span>
          </div>
          <div
            class="calc-card placeholder"
            @click="selectCalculator('neck-angle')"
          >
            <div class="calc-icon">
              📐
            </div>
            <h4>Neck Angle</h4>
            <p>Neck angle → saddle height → break angle chain</p>
            <span class="status-badge">Coming Soon</span>
          </div>
          <div
            class="calc-card placeholder"
            @click="selectCalculator('nut-break-angle')"
          >
            <div class="calc-icon">
              🔩
            </div>
            <h4>Nut Break Angle</h4>
            <p>Angled & flat headstock nut downforce geometry</p>
            <span class="status-badge">Coming Soon</span>
          </div>
        </div>
      </div>
    ...
    >
      ...
      <StringTensionPanel v-if="activeCalculator === 'string-tension'" />
    </div>
  </div>
</template>
...
import StringTensionPanel from './string-tension/StringTensionPanel.vue'
...
function getCalculatorTitle(calcId: string): string {
  const titles: Record<string, string> = {
    ...
    'string-tension': '🎵 String Tension Calculator',
    'fret-spacing': '📏 Fret Spacing Calculator',
    'neck-angle': '📐 Neck Angle Calculator',
    'nut-break-angle': '🔩 Nut Break Angle Calculator',
  }
