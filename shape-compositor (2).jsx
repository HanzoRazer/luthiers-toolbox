...
const SHAPES = {
  ...
  braid_trenza: {
    label: "Braid/Trenza", group: "mosaic", special: "braid",
    hint: "N-strand helical braid. Strands float radially on a sin wave — painter's order gives correct over/under per tile. Each strand gets its own material.",
    params: [
    ...
    // Bounding rect — used for hover hit detection only
    draw(ctx, W, H, _p) {
      ctx.rect(-W/2, -H/2, W, H);
    },
    // Full per-tile braid renderer — called by special branch in renderWheel
    // materials: array of wood objects { base, grain, hi, ... }
    // grainFn(ctx, wood, W, H, seed, dir)
    drawBraid(ctx, W, H, p, idx, materials, grainFn, dir, showOut) {
      const N     = Math.round(p.n);
      const gapPx = H * p.gap;
      const strandH = (H - gapPx*(N-1)) / N * p.sw;
      // Maximum radial excursion so strands fill the tile height
      const amp   = (H/2 - strandH/2) * 0.95;
      const lx    = W * p.lean;            // parallelogram lean
      const rnd   = strandH * p.round;    // end rounding px

      // Phase at tile idx — freq cycles per full ring
      // (We don't know ring N here so freq is relative to a nominal 24-tile ring)
      const phi = idx * p.freq * Math.PI * 2 / 24;

      // Per-strand sin position & z-depth (sin value = "toward viewer")
      const sy = si => amp * Math.sin(phi + si * Math.PI*2 / N);
      const sz = si => Math.sin(phi + si * Math.PI*2 / N);  // +1 = on top

      // Paint order: lowest z first (background → foreground)
      const order = Array.from({length:N}, (_,i)=>i).sort((a,b)=>sz(a)-sz(b));

      order.forEach(si => {
        const cy  = sy(si);
        const wood = materials[si % materials.length];

        // Strand path — parallelogram with optional end rounding
        const drawStrand = () => {
          const x0=-W/2, x1=W/2;
          const yT=cy-strandH/2, yB=cy+strandH/2;
          if(rnd < 1) {
            ctx.moveTo(x0+lx, yT);
            ctx.lineTo(x1+lx, yT);
            ctx.lineTo(x1-lx, yB);
            ...
            // Rounded parallelogram ends
            const r = Math.min(rnd, strandH/2*0.99);
            ctx.moveTo(x0+lx+r, yT);
            ctx.lineTo(x1+lx-r, yT);
            ctx.quadraticCurveTo(x1+lx, yT, x1+lx, yT+r);
            ctx.lineTo(x1-lx, yB-r);
            ctx.quadraticCurveTo(x1-lx, yB, x1-lx-r, yB);
            ctx.lineTo(x0-lx+r, yB);
            ctx.quadraticCurveTo(x0-lx, yB, x0-lx, yB-r);
            ctx.lineTo(x0+lx, yT+r);
            ctx.quadraticCurveTo(x0+lx, yT, x0+lx+r, yT);
            ctx.closePath();
          }
        };
        ...
        // Fill + grain
        ctx.save();
        ctx.beginPath(); drawStrand(); ctx.clip();
        ctx.fillStyle = wood.base;
        ctx.fillRect(-W*1.5, cy-strandH*1.5, W*3, strandH*3);
        grainFn(ctx, wood, W*2, strandH*2.5, si*41+idx, dir);
        ctx.restore();

        // Top-edge highlight (simulates 3D curvature of round strand)
        ctx.save();
        ctx.beginPath(); drawStrand(); ctx.clip();
        const hi = ctx.createLinearGradient(0, cy-strandH/2, 0, cy+strandH/2);
        hi.addColorStop(0, "rgba(255,240,200,0.18)");
        hi.addColorStop(0.3,"rgba(255,240,200,0.05)");
        hi.addColorStop(1, "rgba(0,0,0,0.22)");
        ctx.fillStyle=hi; ctx.fillRect(-W*1.5,cy-strandH,W*3,strandH*2);
        ctx.restore();

        // Under-edge drop shadow onto strand below
        if(sz(si) > 0) { // only for "top" strands
          ctx.save();
          ctx.beginPath(); drawStrand();
          const sh = ctx.createLinearGradient(0, cy+strandH/2-2, 0, cy+strandH/2+4);
          sh.addColorStop(0,"rgba(0,0,0,0.28)"); sh.addColorStop(1,"transparent");
          ctx.strokeStyle=sh; ctx.lineWidth=4; ctx.stroke();
          ctx.restore();
        }
...
function renderWheel(canvas, state) {
  ...
  for(let i=0;i<N;i++){
    ...
    } else if(shapeDef.special==="braid"){
      // Multi-material strand renderer
      const braidMats=[wById(matFg),wById(matAlt),wById(matC),wById(matBg)];
      const grainFn=(c,wood,w,h,seed,dir)=>{
        c.fillStyle=wood.base; c.fillRect(-w,-h,w*2,h*2);
        fillGrain(c,wood,w,h,seed,dir);
      };
      shapeDef.drawBraid(ctx,tileW,tileH,params,i,braidMats,grainFn,grainDir,showOutlines);
      ctx.beginPath();
      shapeDef.draw(ctx,tileW,tileH,params,i);
      ctx.save(); ctx.clip();
...
function ShapeThumb({ shapeKey, selected, onClick }) {
  ...
  useEffect(()=>{
    ...
    if(def.special==="braid"){
      const cols=["#1a1008","#f2e8c8","#4a1e10"],W2=36,H2=26,N2=3,amp2=H2/2*0.75,lx2=W2*0.2;
      cols.forEach((col,si)=>{
        const cy=amp2*Math.sin(si*Math.PI*2/N2),sH=H2/N2*0.8;
        ctx.beginPath();
        ctx.moveTo(-W2/2+lx2,cy-sH/2);ctx.lineTo(W2/2+lx2,cy-sH/2);
        ctx.lineTo(W2/2-lx2,cy+sH/2);ctx.lineTo(-W2/2-lx2,cy+sH/2);ctx.closePath();
        ctx.fillStyle=col;ctx.fill();
        ctx.strokeStyle=selected?"#e8c87a":"#2a1a0a";ctx.lineWidth=0.7;ctx.stroke();
      });
    } else {
...
export default function ShapeCompositor() {
  ...
  const PRESETS = [
    ...
    { name:"Braid 3-str",shape:"braid_trenza",params:{n:3,freq:2.5,sw:0.82,gap:0.06,lean:0.22,round:0.0},N:24,gap:1.0,matFg:"ebony",matBg:"ebony",matAlt:"maple",matC:"rosewood",altMode:"ab",ringW:9 },
    { name:"Braid 4-str",shape:"braid_trenza",params:{n:4,freq:3.0,sw:0.80,gap:0.05,lean:0.18,round:0.12},N:24,gap:1.0,matFg:"ebony",matBg:"ebony",matAlt:"maple",matC:"koa",altMode:"abc",ringW:10 },
    { name:"Braid Pearl", shape:"braid_trenza",params:{n:3,freq:2.0,sw:0.85,gap:0.04,lean:0.28,round:0.2},N:32,gap:1.0,matFg:"mop",matBg:"ebony",matAlt:"abalone",matC:"ebony",altMode:"abc",ringW:8 },
    { name:"Braid Koa",   shape:"braid_trenza",params:{n:2,freq:4.0,sw:0.88,gap:0.08,lean:0.35,round:0.3},N:20,gap:1.0,matFg:"koa",matBg:"ebony",matAlt:"maple",matC:"walnut",altMode:"ab",ringW:7 },
  ];
