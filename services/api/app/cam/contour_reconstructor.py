"""
Contour Reconstruction System
==============================

Chains primitive DXF geometry (LINE + SPLINE) into closed LWPOLYLINE loops.
Solves the problem where CAD drawings use disconnected segments instead of closed paths.

Phase 3.1 Implementation:
- LINE chaining with tolerance-based endpoint matching
- SPLINE adaptive sampling (converts curves to polyline segments)
- Graph-based cycle detection
- Island/hole detection (outer loop vs inner loops)

Algorithm:
1. Build connectivity graph from endpoints
2. Find cycles using depth-first search
3. Classify loops (outer vs holes based on area + winding)
4. Return List[Loop] format for adaptive pocket engine
"""
