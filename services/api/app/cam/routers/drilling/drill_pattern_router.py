"""Drill Pattern Router - Pattern-based drilling (grid, circle, line).

Provides:
- POST /pattern/gcode - Generate pattern drilling G-code
- GET /pattern/info - Get pattern drilling information

Total: 2 routes for pattern-based drilling.
...
router = APIRouter(prefix="/pattern", tags=["CAM", "Drilling", "Pattern"])
...
class Pattern(BaseModel):
    """Pattern definition for drilling."""
...
def _generate_points(p: Pattern) -> List[tuple]:
    """Generate drilling points from pattern specification."""
    pts = []
