...
def validate_dxf_filename(filename: Optional[str]) -> None:
    ...
    if not filename.lower().endswith(".dxf"):
        raise DXFValidationError(f"Only .dxf files are supported (got: {filename})")
...
def validate_file_size(file: UploadFile) -> int:
    ...
    # Use >= to match security-spec behavior and test expectations.
    if file_size >= MAX_DXF_FILE_SIZE_BYTES:
        logger.warning(
            ...
            extra={
                ...
                # Avoid LogRecord reserved attribute collision ("filename").
