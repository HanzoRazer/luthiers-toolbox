...
def generate_advisory_image(
    ...
    Request tracking: Pass X-Request-ID header for correlation.
    """
    start_time = time.perf_counter()
    
    # Get request_id from header if available
    request_id = None
    if request and hasattr(request, 'headers'):
        request_id = request.headers.get('X-Request-ID') or request.headers.get('x-request-id')
    
    # Compute prompt hash for duplicate detection
    ...
    if not result.success or not result.images:
        ...
        if request_id:
            ...
            req_store.save_record(RequestRecord(
                request_id=request_id,
                action="generate",
                endpoint="/api/advisory/generate/image",
                prompt=req.prompt,
    ...
    # Extract provider_request_id if available
    provider_request_id = None
    if generated._result and hasattr(generated._result, 'meta'):
        meta = getattr(generated._result, 'meta', {}) or {}
        provider_request_id = meta.get('request_id') or meta.get('id')
    ...
    # Create advisory asset with request tracking
    asset = AdvisoryAsset(
        ...
        meta={
            ...
            "engine_request_id": result.request.request_id,
        },
    )
    ...
    # Save request record for correlation
    if request_id:
        req_store = get_request_store()
        req_store.save_record(RequestRecord(
            request_id=request_id,
...
def review_asset(
    ...
    If run_id is provided and approved=True, the asset will be attached to that run.
    ...
    if request and hasattr(request, 'headers'):
        review_request_id = request.headers.get('X-Request-ID') or request.headers.get('x-request-id')
    ...
    asset.review_request_id = review_request_id  # Track which request reviewed this
    ...
    # If approved and run_id provided, attach to the run
    ...
    if req.approved and req.run_id:
        try:
            ...
            ref = runs_attach_advisory(
                ...
                kind="advisory",
                engine_id=asset.provider,
                engine_version=asset.model,
                request_id=review_request_id,
            )
            if ref:
                attached_to_run = req.run_id
                logger.info(
                    "advisory_asset.attached_to_run",
                    extra={
                        "asset_id": asset_id,
                        "run_id": req.run_id,
                    }
            ...
            logger.warning(
                "advisory_asset.attach_to_run_failed",
                extra={
                    "asset_id": asset_id,
                    "run_id": req.run_id,
                    "error": str(e),
    ...
    if review_request_id:
        ...
        req_store.save_record(RequestRecord(
            request_id=review_request_id,
            action="review",
            endpoint=f"/api/advisory/assets/{asset_id}/review",
            asset_id=asset_id,
            success=True,
            duration_ms=int((time.perf_counter() - start_time) * 1000),
            meta={
                ...
                "rating": req.rating,
                "generation_request_id": asset.request_id,  # Link to generation
                "attached_to_run": attached_to_run,
            },
        ))
...
@router.post("/assets/{asset_id}/attach-to-run", response_model=AttachToRunResponse)
def attach_asset_to_run(
    asset_id: str,
    req: AttachToRunRequest,
    request: Request = None,
) -> AttachToRunResponse:
    """
    Attach an approved advisory asset to a run.
    
    GOVERNANCE: Only approved assets can be attached to runs.
    This creates an append-only advisory_inputs reference on the run.
    
    The asset must be:
    1. Reviewed (reviewed=True)
    2. Approved (approved_for_workflow=True)
    
    Use this when you want to associate a Vision Engine image with
    a specific RMOS run for reference in the workflow.
    """
    # Get request_id from header
    attach_request_id = None
    if request and hasattr(request, 'headers'):
        attach_request_id = request.headers.get('X-Request-ID') or request.headers.get('x-request-id')
    ...
    # Governance check: must be reviewed and approved
    if not asset.reviewed:
        raise HTTPException(
            status_code=400,
            detail={
                "error": "ASSET_NOT_REVIEWED",
                "asset_id": asset_id,
                "message": "Asset must be reviewed before attaching to a run",
            }
        )
    
    if not asset.approved_for_workflow:
        raise HTTPException(
            status_code=400,
            detail={
                "error": "ASSET_NOT_APPROVED",
                "asset_id": asset_id,
                "message": "Asset must be approved for workflow before attaching to a run",
            }
        )
    ...
    try:
        ...
        if ref:
            ...
            return AttachToRunResponse(
                success=True,
                asset_id=asset_id,
                run_id=req.run_id,
                advisory_ref_created=True,
                message=f"Asset {asset_id} attached to run {req.run_id}",
            )
        else:
            return AttachToRunResponse(
                success=False,
                asset_id=asset_id,
                run_id=req.run_id,
                advisory_ref_created=False,
                message="Failed to attach - run may not exist",
            )
    except HTTPException:
        raise
    except Exception as e:
        logger.error(
            "advisory_asset.attach_to_run_error",
            extra={
                "asset_id": asset_id,
                "run_id": req.run_id,
                "error": str(e),
...
def get_asset_runs(asset_id: str):
    """
    Get all runs that reference this advisory asset.
    
    Searches run artifacts for advisory_inputs containing this asset_id.
    Useful for tracing which workflow runs used a particular image.
    """
    store = get_advisory_store()
    asset = store.get_asset(asset_id)
...
def get_run_advisories(run_id: str):
    """
    Get all advisory assets attached to a run.
    
    Returns advisory references from the run plus full asset details
    where available.
    """
    try:
        ...
        for ref in (run.advisory_inputs or []):
            asset = store.get_asset(ref.advisory_id)
            ...
            advisories.append({
                ...
                "approved": asset.approved_for_workflow if asset else None,
                "rating": asset.rating if asset else None,
            })
...
def check_duplicates(req: DuplicateCheckRequest) -> DuplicateCheckResponse:
    """
    Check for duplicate or similar images before generating.
    
    Prevents wasting money on redundant generations by finding:
    - Exact matches: Same prompt (by hash)
    - Similar: High word overlap (by threshold)
    
    Recommendation:
    - proceed: No duplicates found, safe to generate
    - review_existing: Similar images exist, review them first
    - skip: Exact duplicate exists, don't regenerate
    """
    store = get_advisory_store()
    ...
    if req.check_exact:
        exact_assets = store.find_by_prompt_hash(prompt_hash, limit=req.max_results)
        for asset in exact_assets:
            exact_matches.append(SimilarityMatch(
                ...
                match_type="exact",
                prompt=asset.prompt,
                asset_url=f"/api/advisory/assets/{asset.asset_id}/content",
            ))
    
    # Check for similar prompts
    if req.check_similar:
        similar_results = store.find_similar_prompts(
            req.prompt,
            threshold=req.threshold,
            limit=req.max_results,
        )
        for asset, score in similar_results:
            # Skip if already in exact matches
            if any(m.asset_id == asset.asset_id for m in exact_matches):
                continue
            
            match_type = "near_duplicate" if score > 0.9 else "similar"
...
# =============================================================================
# REQUEST CORRELATION (Feature 21)
# =============================================================================

@router.get("/requests/{request_id}")
def get_request_by_id(request_id: str):
    """
    Get all records for a global request_id.
    
    Shows the full chain: generate → review → export
    tied together by the X-Request-ID header.
    """
    req_store = get_request_store()
    records = req_store.find_by_request_id(request_id)
    
    if not records:
        raise HTTPException(
            status_code=404,
            detail={"error": "REQUEST_NOT_FOUND", "request_id": request_id}
        )
...
@router.get("/requests/asset/{asset_id}")
def get_requests_for_asset(asset_id: str):
    """
    Get all request records for an asset.
    
    Shows the full lifecycle: when generated, when reviewed.
    """
    req_store = get_request_store()
    records = req_store.find_by_asset_id(asset_id)
    
    # Also get the asset to show its stored request IDs
    store = get_advisory_store()
    asset = store.get_asset(asset_id)
    
    return {
        "asset_id": asset_id,
        "generation_request_id": asset.request_id if asset else None,
        "provider_request_id": asset.provider_request_id if asset else None,
        "review_request_id": asset.review_request_id if asset else None,
        "records": [r.model_dump() for r in records],
        "total": len(records),
    }
...
def list_recent_requests(
    limit: int = Query(default=50, ge=1, le=200),
    action: Optional[str] = Query(default=None, description="Filter by action: generate, review, export"),
):
    """
    ...
    """
    req_store = get_request_store()
    records = req_store.list_recent(limit=limit)
