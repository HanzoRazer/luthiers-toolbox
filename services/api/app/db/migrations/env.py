...
# Art Studio + CAM telemetry (SQLite schema audit 2026-03-28)
from app.db.alembic_models import (  # noqa: F401
    CamLogRun,
    CamLogSegment,
    RosetteCompareRisk,
    RosetteJob,
...
# Tables autogenerated for revision "initial schema from sqlite" (exclude workflow/auth noise)
_AUTOGEN_ART_CAM_TABLES = frozenset(
    {
        "rosette_jobs",
        "rosette_presets",
        "rosette_compare_risk",
        "runs",
        "segments",
    }
)
...
def include_object(object, name, type_, reflected, compare_to):  # noqa: ARG001
    """Limit autogenerate to art_studio + cam_logs tables (see schema audit JSON)."""
    if type_ == "table":
        return name in _AUTOGEN_ART_CAM_TABLES
    tbl = getattr(object, "table", None)
    if tbl is not None and getattr(tbl, "name", None):
        return tbl.name in _AUTOGEN_ART_CAM_TABLES
    return True
