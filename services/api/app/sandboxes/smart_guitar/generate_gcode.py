...
Generates G-code from smart_guitar_cavity_map.json through the full CAM pipeline:
1. Load cavity map → SmartGuitarSpec
2. Generate CAM plan (SmartCamPlan)
3. Resolve geometry (positioned DXF templates)
4. Generate G-code for each toolpath operation

Usage:
    python -m app.sandboxes.smart_guitar.generate_gcode

Output:
    services/api/exports/smart_guitar_[timestamp].nc
"""
...
def generate_smart_guitar_gcode(
    ...
    Args:
        cavity_map_path: Path to smart_guitar_cavity_map.json
...
if __name__ == "__main__":
    ...
    handedness = "RH"
    if len(sys.argv) > 1 and sys.argv[1].upper() in ["LH", "LEFT"]:
        handedness = "LH"

    print("=" * 60)
    print("Smart Guitar G-code Generator")
    print("=" * 60)

    gcode = generate_smart_guitar_gcode(handedness=handedness)
