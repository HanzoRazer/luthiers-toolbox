...
- Prints a humanΓÇæreadable report by default.
...
def approx_arc_bbox(summary: Summary, start: Tuple[float, float, float], end: Tuple[float, float, float],
    ...
    # Conservative: include start, end, and centerΓÇæpoint; this will overΓÇæapproximate a bit.
...
def parse_gcode(path: str, pretty: bool = False, validate: bool = False) -> Tuple[Summary, List[Move]]:
    ...
    if file_size > 100 * 1024 * 1024:  # 100MB
        print(f"ΓÜá∩╕Å  Warning: Large file ({file_size / 1024 / 1024:.1f} MB) - parsing may take time")
...
def fmt_num(v: Optional[float], units: str = "", none: str = "ΓÇö") -> str:
    if v is None:
        return none
    if units:
        return f"{v:.4f} {units}"
...
def print_report(summary: Summary, moves: List[Move], pretty: bool = False) -> None:
    ...
    print(f"≡ƒôè Size:           {summary.file_size_bytes:,} bytes ({summary.file_size_bytes / 1024:.1f} KB)")
    print()
    print("ΓÜÖ∩╕Å  Program Configuration")
    print("ΓöÇ" * 60)
    ...
    print()
    print("≡ƒöº Motion Statistics")
    print("ΓöÇ" * 60)
    print(f"Total motions:    {summary.motion_count:,}")
    print(f"  Γö£ΓöÇ Rapids:      {summary.rapid_count:,} moves")
    print(f"  Γö£ΓöÇ Feeds:       {summary.feed_count:,} moves")
    print(f"  ΓööΓöÇ Arcs:        {summary.arc_count:,} moves")
    print()
    ...
    print(f"Tools:            {', '.join(summary.tools) if summary.tools else 'ΓÇö'}")
    
    if summary.estimated_time_minutes:
        hours = int(summary.estimated_time_minutes / 60)
        mins = int(summary.estimated_time_minutes % 60)
        if hours > 0:
            print(f"ΓÅ▒∩╕Å  Estimated time:  {hours}h {mins}m (cut time only)")
        else:
            print(f"ΓÅ▒∩╕Å  Estimated time:  {mins} minutes (cut time only)")
    print()
    print("≡ƒôÅ Program Bounds")
    print("ΓöÇ" * 60)
    ...
    print()
    print("≡ƒôÉ Total Travel")
    print("ΓöÇ" * 60)
    print(f"Total distance:   {fmt_num(summary.length_total, u)}")
    print(f"  Γö£ΓöÇ Rapids:      {fmt_num(summary.length_rapid, u)}")
    print(f"  ΓööΓöÇ Feeds:       {fmt_num(summary.length_feed, u)}")
    
    # Print warnings
    if summary.warnings:
        print()
        print("ΓÜá∩╕Å  Warnings")
        print("ΓöÇ" * 60)
    ...
    if pretty:
        print("\nFirst 25 motions (startΓåÆend):")
        for mv in moves[:25]:
            code = mv.code
            tag = "RAPID" if mv.rapid else ("ARC" if mv.arc else "FEED")
            print(f"[{mv.n:>6}] {code:<3} {tag:<5}  "
                  f"({mv.start[0]:.4f},{mv.start[1]:.4f},{mv.start[2]:.4f}) ΓåÆ "
                  f"({mv.end[0]:.4f},{mv.end[1]:.4f},{mv.end[2]:.4f})  "
                  f"F={fmt_num(mv.feed)} S={fmt_num(mv.spindle)} T={mv.tool or 'ΓÇö'}")
...
def main(argv: Optional[List[str]] = None) -> int:
    p = argparse.ArgumentParser(
        description="Read & summarize a GΓÇæcode (.nc/.gcode) file",
...
Integration with The Production Shop:
  ...
  automated G-code analysis in CAM workflows.
    ...
    if not file_path.exists():
        print(f"Γ¥î Error: File not found: {file_path}", file=sys.stderr)
        return 1
