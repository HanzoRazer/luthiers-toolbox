...
from .hashing import summarize_request, compute_feasibility_hash, sha256_of_obj
...
# =============================================================================
# Phase 5: On-demand AI-assisted explanation (advisory-only)
# =============================================================================
...
def explain_run_on_demand(
    run_id: str,
    force: bool = Query(False, description="Regenerate and attach a new assistant explanation even if one exists."),
) -> Dict[str, Any]:
    """
    ...
    Returns:
        ...
        - run_id: The run ID
        - explanation_status: "READY" or "ERROR"
        - attachment: The attachment metadata
        - explanation: The full explanation payload
    """
    run = get_run(run_id)
    if run is None:
    ...
    if existing is not None and not force:
        # Idempotent: return existing attachment reference
        return {
            "ok": True,
            "run_id": run.run_id,
    ...
    try:
        grounded_hash = sha256_of_obj(
            {
                "run_id": run.run_id,
                "risk_level": getattr(run.decision, "risk_level", None) if run.decision else None,
                "rules_triggered": (run.feasibility or {}).get("rules_triggered", []),
    ...
    # Store as content-addressed JSON attachment
