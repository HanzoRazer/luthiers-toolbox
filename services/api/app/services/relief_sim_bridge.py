...
Relief Simulation Bridge - Mesh-ish material removal estimation

Part of Phase 24.3: Relief Sim Bridge Backend
Repository: HanzoRazer/luthiers-toolbox
Created: January 2025

This module provides Z-aware material removal simulation for relief toolpaths:
- Rasterizes moves onto a grid
- Tracks min Z per cell (floor depth)
- Estimates load index per cell (|depth| × path length)
- Emits issues (thin floor, high load) + overlays for visualization

Plugs into existing risk analytics + backplot overlay infrastructure.
"""
...
def _moves_to_segments(moves: List[ReliefMove]) -> List[Tuple[float, float, float, float, float]]:
    ...
    Returns:
        List of (x0, y0, x1, y1, z_mid) tuples for each segment.
    """
...
def _estimate_grid_bounds(
    ...
    """
    Estimate grid dimensions and origin from segments.
...
def _rasterize_segments_to_grid(
    ...
    """
    Rasterize segments onto grid with floor depth and load accumulation.
    
    Returns:
        (floor_depth, load_accum) - 2D arrays (height, width)
...
def run_relief_sim_bridge(payload: ReliefSimIn) -> ReliefSimOut:
    ...
    Args:
        payload: ReliefSimIn with moves, stock thickness, thresholds
        
    Returns:
        ReliefSimOut with issues, overlays, and statistics
    """
