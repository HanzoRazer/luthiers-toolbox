...
# =============================================================================
# Pydantic-Based Simple API (merged from headstock_break_angle.py)
# =============================================================================
# This provides a simpler, whole-headstock calculation for common use cases.
# For per-string analysis, use analyze_headstock() instead.

from pydantic import BaseModel, Field


# Thresholds (shared with the detailed analyzer)
TOO_SHALLOW_DEG = 3.0
OPTIMAL_MIN_DEG = 5.0
...
class HeadstockBreakAngleInput(BaseModel):
    """Input parameters for nut break angle calculation (simple API)."""
...
class HeadstockBreakAngleResult(BaseModel):
    """Output of the nut break angle calculation (simple API)."""
...
def calculate_headstock_break_angle(inp: HeadstockBreakAngleInput) -> HeadstockBreakAngleResult:
    """
    Compute the effective string break angle at the nut (simple API).
    ...
    if inp.headstock_type == "flat":
        angle_deg = _calc_flat_headstock_simple(inp, flags)
    else:
        angle_deg = _calc_angled_headstock_simple(inp, flags)
    ...
    if angle_deg < TOO_SHALLOW_DEG:
        ...
        flags.append(NutRiskFlag(
            ...
            severity="warning",
            message=(
                f"Nut break angle {angle_deg:.1f} deg is below {TOO_SHALLOW_DEG} deg. "
        ...
        flags.append(NutRiskFlag(
            ...
            severity="warning",
            message=(
                f"Nut break angle {angle_deg:.1f} deg exceeds {TOO_STEEP_DEG} deg. "
        ...
        if angle_deg < OPTIMAL_MIN_DEG:
            ...
            flags.append(NutRiskFlag(
                ...
                message=(
                    ...
                    f"nut break angle ({angle_deg:.1f} deg). A string tree providing "
    ...
    if rating == "too_shallow":
        if inp.headstock_type == "flat":
            ...
            recommendation = (
                f"Increase headstock angle to achieve at least {target_angle} deg "
                f"nut break angle. Current headstock pitch: {inp.headstock_angle_deg} deg."
            )
    elif rating == "too_steep":
        recommendation = (
            "Consider using taller tuner posts or reducing the headstock "
            f"pitch angle (currently {inp.headstock_angle_deg} deg) to bring the "
            f"nut break angle below {TOO_STEEP_DEG} deg."
        )
...
def _calc_angled_headstock_simple(
...
def _calc_flat_headstock_simple(
    inp: HeadstockBreakAngleInput,
    flags: List[NutRiskFlag],
) -> float:
    """
