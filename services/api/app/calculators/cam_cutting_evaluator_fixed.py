"""CAM cutting evaluation ΓÇö chipload, heat, deflection, rim-speed, kickback, bite.Pure calculation functions with zero FastAPI dependency.Extracted from ``calculators_consolidated_router.py`` (WP-3)."""
...
class RimSpeedResult:
    ...
    within_limits: bool    max_recommended_m_per_min: Optional[float] = None
    message: str = ""@dataclass
...
def calculate_chipload(
    ...
    min_chipload: float = 0.05, max_chipload: float = 0.25,) -> ChiploadResult:
    ...
    return ChiploadResult(        chipload_mm=chipload, in_range=in_range,        min_recommended_mm=min_chipload, max_recommended_mm=max_chipload, message=message,    )@safety_critical
def calculate_heat_risk(
    chipload_mm: Optional[float], rpm: float, depth_of_cut_mm: float,    material_heat_sensitivity: float = 0.5,) -> HeatResult:
    ...
    heat_risk = min(1.0, max(0.0, heat_risk))    if heat_risk < 0.3:    
    return HeatResult(heat_risk=heat_risk, category="COOL", message="Low heat risk")    elif heat_risk < 0.6:    
    return HeatResult(heat_risk=heat_risk, category="WARM", message="Moderate heat risk - monitor for burns")
    else:    
    return HeatResult(heat_risk=heat_risk, category="HOT", message="High heat risk - reduce RPM or increase feed")@safety_critical
def calculate_deflection(    tool_diameter_mm: float, depth_of_cut_mm: float,    width_of_cut_mm: Optional[float], stickout_mm: float = 25.0,) -> DeflectionResult:
    ...
    deflection_mm = (stickout_ratio ** 3) * force_proxy * 0.0001    if deflection_mm < 0.05:    
    return DeflectionResult(deflection_mm=deflection_mm, risk="GREEN", message=f"Low deflection ({deflection_mm:.4f}mm)")    elif deflection_mm < 0.15:    
    return DeflectionResult(deflection_mm=deflection_mm, risk="YELLOW", message=f"Moderate deflection ({deflection_mm:.4f}mm)")
    else:    
    return DeflectionResult(deflection_mm=deflection_mm, risk="RED", message=f"High deflection ({deflection_mm:.4f}mm)")@safety_critical
def calculate_rim_speed(blade_diameter_mm: float, rpm: float, max_rim_speed: float = 80.0) -> RimSpeedResult:
    ...
    within_limits = rim_speed <= max_rim_speed    if within_limits:    
    ...
    return RimSpeedResult(        surface_speed_m_per_min=rim_speed, within_limits=within_limits,        max_recommended_m_per_min=max_rim_speed, message=message,    )@safety_critical
def calculate_bite_per_tooth(
    feed_mm_min: float, rpm: float, tooth_count: int,    min_bite: float = 0.05, max_bite: float = 0.20,) -> BitePerToothResult:
    ...
    in_range = min_bite <= bite <= max_bite    if bite < min_bite:    
        message = f"Bite {bite:.4f}mm below min - rubbing risk"    elif bite > max_bite:    
        message = f"Bite {bite:.4f}mm exceeds max - tooth damage risk"
    else:    
        message = f"Bite {bite:.4f}mm within recommended range"
    return BitePerToothResult(        bite_mm=bite, in_range=in_range,        min_recommended_mm=min_bite, max_recommended_mm=max_bite, message=message,    )@safety_critical
def calculate_kickback_risk(    depth_of_cut_mm: float, blade_diameter_mm: float,    tooth_count: int, feed_mm_min: float,) -> KickbackRiskResult:
    ...
    risk_score = min(1.0, max(0.0, risk_score))    if risk_score < 0.3:    
    return KickbackRiskResult(risk_score=risk_score, category="LOW", message="Low kickback risk")    elif risk_score < 0.6:    
    return KickbackRiskResult(risk_score=risk_score, category="MEDIUM", message="Moderate kickback - ensure workholding")
    else:    
    return KickbackRiskResult(risk_score=risk_score, category="HIGH", message="HIGH kickback risk - reduce depth/feed")@safety_critical
def evaluate_cut_operation(    tool_id: str,    material_id: str,    tool_kind: str,
    feed_mm_min: float,    rpm: float,    depth_of_cut_mm: float,    width_of_cut_mm: Optional[float] = None,    tool_diameter_mm: float = 6.0,    flute_count: int = 2,) -> Dict[str, Any]:
    """Evaluate a cut operation and return all calculator results as a dict.    Returns a flat dict suitable for constructing ``CAMCalculatorBundleResponse``.
    """
    warnings: List[str] = []
