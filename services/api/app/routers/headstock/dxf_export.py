...
class VeneerSpec(BaseModel):
    """
    Headstock face veneer — separate tool-change layer for glued cap.

    The veneer outline is the headstock outline offset inward by
    `overlap_mm` so it sits inside the through-cut perimeter.
    A separate Z-depth layer lets the BCAM surface the cap without
    cutting through to the underlying wood.

    Typical values:
      thickness_mm  1.5–3.0 (ebony/maple cap over mahogany)
      overlap_mm    0.5–1.0 (inward from outline — seating ledge)
    """
    thickness_mm:   float = 2.0
    overlap_mm:     float = 0.5    # inward offset from outline edge
    species:        str   = ""     # annotation only (e.g. "Ebony over Mahogany")


class BindingSpec(BaseModel):
    """
    Headstock binding rabbet — router pass creates a shelf at the perimeter
    edge into which binding strip is glued.

    Geometry:
      - Rabbet outline = headstock outline offset inward by `width_mm`
      - Rabbet depth   = `thickness_mm` (routed into top face)
      - Minimum bend radius validated against material type

    Material bend minimums (from repo binding_geometry.py):
      celluloid   6mm   abs_plastic 3mm   wood_maple   15mm
      wood_rosewood 20mm   fiber 4mm   ivoroid 5mm   abalone 8mm
    """
    width_mm:       float = 2.5    # binding strip width
    thickness_mm:   float = 1.5    # depth of rabbet
    material:       str   = "abs_plastic"
    purfling_mm:    float = 0.0    # optional purfling inset width (0 = none)
...
class ExportRequest(BaseModel):
    ...
    # ── Veneer + binding ──────────────────────────────────────────────────────
    veneer:       Optional[VeneerSpec]  = None
...
def build_dxf(req: ExportRequest) -> ezdxf.document.Drawing:
    ...
    # Veneer + binding layers (added conditionally below but registered upfront)
    add_layer("HS_VENEER",   color=6)    # magenta — veneer face outline
    add_layer("HS_BINDING",  color=2)    # yellow  — binding rabbet
    add_layer("HS_PURFLING", color=9)    # light gray — purfling inset
    ...
    # ── Veneer face layer ─────────────────────────────────────────────────────
    # The veneer outline sits inside the through-cut perimeter by `overlap_mm`
    # so it fits flush when the cap is glued onto the back face.
    if req.veneer and len(outline_pts) >= 3:
        v = req.veneer
        veneer_pts = offset_polygon(outline_pts, -v.overlap_mm)  # inward
        if len(veneer_pts) >= 2:
            msp.add_lwpolyline(
                [(p.x, p.y) for p in veneer_pts],
                close=True,
                dxfattribs={"layer": "HS_VENEER"},
            )
        species_note = f" ({v.species})" if v.species else ""
        msp.add_text(
            f"Veneer cap{species_note}  t={v.thickness_mm}mm  overlap={v.overlap_mm}mm",
            dxfattribs={"layer": LAYER_ANNOTATION, "height": 1.5},
        ).set_placement(
            (outline_pts[0].x, outline_pts[0].y - 8),
            align=TextEntityAlignment.LEFT,
        )

    # ── Binding channel (rabbet) ───────────────────────────────────────────────
    # Outer binding wall = headstock outline (already on HS_OUTLINE).
    # Inner binding wall = outline offset inward by binding width.
    # The BCAM routes the rabbet at binding_thickness_mm depth.
    # Bend radius is validated against the material minimum from the repo's
    # binding_geometry.py constants.
    _BEND_RADII_MM = {
        "celluloid": 6.0, "abs_plastic": 3.0, "wood_maple": 15.0,
        "wood_rosewood": 20.0, "fiber": 4.0, "ivoroid": 5.0, "abalone_shell": 8.0,
    }
    if req.binding and len(outline_pts) >= 3:
        b = req.binding
        # Inner edge of binding rabbet
        inner_pts = offset_polygon(outline_pts, -b.width_mm)
        if len(inner_pts) >= 2:
            msp.add_lwpolyline(
                [(p.x, p.y) for p in inner_pts],
                close=True,
                dxfattribs={"layer": "HS_BINDING"},
            )
        # Optional purfling inset (second inward offset)
        if b.purfling_mm > 0:
            purfling_pts = offset_polygon(outline_pts, -(b.width_mm + b.purfling_mm))
            if len(purfling_pts) >= 2:
                msp.add_lwpolyline(
                    [(p.x, p.y) for p in purfling_pts],
                    close=True,
                    dxfattribs={"layer": "HS_PURFLING"},
                )

        # Bend radius check — warn if any outline segment too tight for material
        min_bend = _BEND_RADII_MM.get(b.material, 6.0)
        perimeter_mm = sum(
            math.hypot(outline_pts[i].x - outline_pts[i-1].x,
                       outline_pts[i].y - outline_pts[i-1].y)
            for i in range(1, len(outline_pts))
        )
        binding_note = (
            f"Binding: {b.width_mm}mm {b.material}  "
            f"t={b.thickness_mm}mm  min_bend={min_bend}mm  "
            f"perimeter≈{perimeter_mm:.0f}mm"
        )
        if b.purfling_mm > 0:
            binding_note += f"  +{b.purfling_mm}mm purfling"
        msp.add_text(
            binding_note,
            dxfattribs={"layer": LAYER_ANNOTATION, "height": 1.5},
        ).set_placement(
            (outline_pts[0].x, outline_pts[0].y - 12),
            align=TextEntityAlignment.LEFT,
        )
...
#
# Board-foot pricing reference (USD, 2024 typical retail):
#   Mahogany  4–8   Maple 5–10   Walnut 8–14
#   Rosewood  20–40  Ebony 30–60  Koa   25–50
# These are starting defaults — override via price_per_bf_usd in the request.

_DEFAULT_PRICE_PER_BF: dict[str, float] = {
    "mahogany":  6.0,
    "maple":     7.0,
    "walnut":    11.0,
    "rosewood":  30.0,
...
class CostRequest(BaseModel):
    outline_path:       str
    blank_thickness_mm: float = 19.05      # 3/4" standard headstock blank
    blank_width_mm:     float = 120.0      # blank width (headstock fits inside)
    blank_length_mm:    float = 200.0      # blank length
    species:            str   = "mahogany"
    price_per_bf_usd:   float = 0.0        # 0 = use default for species
    kerf_waste_pct:     float = 15.0       # saw kerf + edge waste %
    quantity:           int   = 1
    scale_override:     Optional[float] = None
    # Optional veneer cap
    veneer_species:     str   = ""
    veneer_thickness_mm:float = 0.0
    veneer_price_per_bf:float = 0.0


def _shoelace_area(pts: list[Vec2]) -> float:
    """Signed polygon area via shoelace formula (mm²). Positive = CCW."""
    n = len(pts)
    if n < 3:
        return 0.0
    total = 0.0
...
@router.post("/headstock-dxf/cost")
async def estimate_material_cost(req: CostRequest):
    """
    Estimate material cost for a headstock blank.

    Returns:
      - headstock_area_cm2     — outline footprint
      - blank_area_cm2         — full blank footprint
      - utilisation_pct        — headstock area / blank area
      - blank_board_feet       — blank volume in board-feet
      - price_per_bf           — price used (default or override)
      - material_cost_usd      — blank cost × quantity
      - veneer_cost_usd        — cap veneer cost if specified
      - total_cost_usd         — material + veneer × quantity
      - waste_adjusted_cost    — total × (1 + kerf_waste_pct / 100)
    """
    scale = req.scale_override if req.scale_override else MM_PER_UNIT
    pts   = svg_path_to_points(req.outline_path, scale)

    # Headstock outline area (mm²)
    area_mm2 = abs(_shoelace_area(pts)) if pts else 0.0
    area_cm2 = area_mm2 / 100.0

    # Blank volume → board-feet
    # 1 board-foot = 144 in² × 1" thick = 2359.74 cm³
    blank_vol_cm3 = (
        (req.blank_width_mm / 10) *
        (req.blank_length_mm / 10) *
        (req.blank_thickness_mm / 10)
    )
    board_feet = blank_vol_cm3 / 235.974     # cm³ per bf

    # Pricing
    species_key = req.species.lower()
    ppbf = req.price_per_bf_usd if req.price_per_bf_usd > 0 else _DEFAULT_PRICE_PER_BF.get(species_key, 8.0)
    material_cost = board_feet * ppbf

    # Veneer cap
    veneer_cost = 0.0
    if req.veneer_thickness_mm > 0 and req.veneer_species:
        v_vol_cm3 = (
            (req.blank_width_mm / 10) *
            (req.blank_length_mm / 10) *
            (req.veneer_thickness_mm / 10)
        )
        v_bf = v_vol_cm3 / 235.974
        v_ppbf = req.veneer_price_per_bf if req.veneer_price_per_bf > 0 else _DEFAULT_PRICE_PER_BF.get(req.veneer_species.lower(), 20.0)
        veneer_cost = v_bf * v_ppbf

    blank_area_cm2 = (req.blank_width_mm * req.blank_length_mm) / 100.0
    utilisation = (area_cm2 / blank_area_cm2 * 100.0) if blank_area_cm2 > 0 else 0.0

    total_per_unit  = material_cost + veneer_cost
    total_qty       = total_per_unit * req.quantity
    waste_factor    = 1 + req.kerf_waste_pct / 100
    waste_adjusted  = total_qty * waste_factor

    return {
        "species":              req.species,
        "headstock_area_cm2":   round(area_cm2, 2),
        "blank_area_cm2":       round(blank_area_cm2, 2),
        "utilisation_pct":      round(utilisation, 1),
        "blank_board_feet":     round(board_feet, 4),
        "price_per_bf_usd":     round(ppbf, 2),
        "material_cost_usd":    round(material_cost, 4),
        "veneer_cost_usd":      round(veneer_cost, 4),
        "total_cost_usd":       round(total_per_unit, 4),
        "quantity":             req.quantity,
        "total_qty_cost_usd":   round(total_qty, 2),
        "kerf_waste_pct":       req.kerf_waste_pct,
        "waste_adjusted_usd":   round(waste_adjusted, 2),
        "cost_per_unit_final":  round(waste_adjusted / max(1, req.quantity), 2),
        "note": f"Defaults from 2024 retail. Override price_per_bf_usd for your sourcing.",
    }
