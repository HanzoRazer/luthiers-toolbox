"""
CAM Metrics Router for M.3 Energy & Heat Model.

Provides energy and heat analysis endpoints that integrate:
- Material database (sce_j_per_mm3, heat_partition)
- Energy model calculations (volume removed, total energy)
...
def thermal_report_md(body: ThermalReportIn) -> StreamingResponse:
    ...
    def _state(val, lim):
        if val <= lim*0.85: return "OK"
        if val <= lim: return "WARN"
        return "OVER"
    ...
    w("## Totals\n")
    w(f"- **Volume removed**: {energy['totals']['volume_mm3']:.0f} mm³  \n")
    w(f"- **Total energy**: {energy['totals']['energy_j']:.1f} J  \n")
...
def thermal_report_bundle(body: ThermalReportIn) -> StreamingResponse:
    ...
    # Bottleneck counts
    caps = energy.get("caps", {"feed_cap": 0, "accel": 0, "jerk": 0, "none": 0})
    ...
    w(f"- Machine: `{body.machine_profile_id}`  \n- Material: `{mat['id']}`  \n")
    w(
        f"- Tool Ø: {body.tool_d:.3f} mm, Step-over: {(body.stepover*100):.1f}%, Step-down: {body.stepdown:.3f} mm\n\n"
    )

    w("## Totals\n")
    w(
        f"- Volume: {energy['totals']['volume_mm3']:.0f} mm³, Energy: {energy['totals']['energy_j']:.1f} J, Time: {ts['total_s']:.1f} s\n\n"
    )
    ...
    w("## Bottleneck share (counts)\n")
    w(
        f"- feed_cap: {caps['feed_cap']}, accel: {caps['accel']}, jerk: {caps['jerk']}, none: {caps['none']}\n\n"
    )
    ...
    if body.include_csv_links:
        ...
        w("  -d '{")
        w(
            f"\"moves\":<PASTE_MOVES_JSON>,\"material_id\":\"{mat['id']}\",\"tool_d\":{body.tool_d},\"stepover\":{body.stepover},\"stepdown\":{body.stepdown},\"job_name\":\"{body.job_name or ''}\""
        )
        w("}' -o ")
        w(f"{energy_stem}.csv\n")
        w("```\n\n")
        ...
        w("  -d '{")
        w(
            f"\"moves\":<PASTE_MOVES_JSON>,\"machine_profile_id\":\"{body.machine_profile_id}\",\"material_id\":\"{mat['id']}\",\"tool_d\":{body.tool_d},\"stepover\":{body.stepover},\"stepdown\":{body.stepdown},\"bins\":{body.bins},\"job_name\":\"{body.job_name or ''}\""
        )
        w("}' -o ")
        w(f"{heat_stem}.csv\n")
        w("```\n\n")
        ...
        w(
            "> **Note:** The `moves.json` file in this bundle contains the ready-to-paste moves array.\n"
        )
