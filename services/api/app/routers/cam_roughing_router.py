...
def generate_roughing_program(req: "RoughReq") -> str:
    """
    Pure generator: returns G-code text for the requested roughing rectangle.
    Single source of truth for roughing toolpath - used by both draft and governed endpoints.
    """
    ...
    lines = [
        ...
        "G1 X0",  # Cut back to near X
        "G1 Y0",  # Cut back to origin
...
def roughing_gcode(req: RoughReq) -> Response:
    """
    Generate simple rectangular roughing G-code (DRAFT lane).
    ...
    """
    program = generate_roughing_program(req)
...
def roughing_gcode_governed(req: RoughReq) -> Response:
    """
    Generate simple rectangular roughing G-code (GOVERNED lane).
