...
from app.workflow.state_machine import (
    WorkflowSession, WorkflowMode, WorkflowState, ActorRole, RiskBucket, FeasibilityResult,
    new_session, set_design, set_context, request_feasibility, store_feasibility,
    approve, reject, require_revision, next_step_hint,
)
...
from app.art_studio.schemas.design_snapshot import DesignContextRefs, SnapshotCreateRequest
...
from .workflow_schemas import (
    ...
    SessionStatusResponse, WorkflowCreatedResponse, FeasibilityResponse,
    ApprovalResponse, SnapshotCreatedResponse, GeneratorInfo,
)

__all__ = [
    "CreateFromPatternRequest", "CreateFromGeneratorRequest", "CreateFromSnapshotRequest", "EvaluateFeasibilityRequest",
    "ApproveSessionRequest", "RejectSessionRequest", "RequestRevisionRequest", "UpdateDesignRequest", "SaveSnapshotRequest",
    "SessionStatusResponse", "WorkflowCreatedResponse", "FeasibilityResponse", "ApprovalResponse", "SnapshotCreatedResponse",
    "GeneratorInfo", "create_workflow_from_pattern", "create_workflow_from_generator", "create_workflow_from_snapshot",
    "evaluate_session_feasibility", "approve_session", "reject_session", "request_session_revision", "update_session_design",
    "save_session_as_snapshot", "list_active_sessions", "get_session_status", "get_available_generators",
]
...
def _build_context_from_refs(refs: Optional[DesignContextRefs]) -> Dict[str, Any]:
    ...
    if refs.material_preset_id:
        ...
        if material:
            context["material"] = {"id": material.id, "name": material.name, "hardness": material.hardness,
                "density_kgm3": material.density_kgm3, "burn_tendency": material.burn_tendency,
                "tearout_tendency": material.tearout_tendency, "recommended_feed_mm_min": material.recommended_feed_mm_min,
    ...
    if refs.tool_preset_id:
        ...
        if tool:
            context["tool"] = {"id": tool.id, "name": tool.name, "diameter_mm": tool.diameter_mm,
                "flute_count": tool.flute_count, "tool_type": tool.tool_type}
...
def create_workflow_from_pattern(
    ...
    # Create session
    index_meta = {"tool_id": context.get("tool_id"), "material_id": context.get("material_id"),
                  "machine_id": context.get("machine_id"), "pattern_id": request.pattern_id,
                  "generator_key": pattern.generator_key}
    ...
    logger.info("art_studio.workflow_from_pattern", extra={"session_id": session.session_id,       "pattern_id": request.pattern_id,       "generator_key": pattern.generator_key,})
    
    return WorkflowCreatedResponse(session_id=session.session_id, mode=session.mode.value,
        state=session.state.value, next_step=next_step_hint(session), warnings=warnings)
...
def create_workflow_from_generator(
    ...
    # Create session
    index_meta = {"tool_id": context.get("tool_id"), "material_id": context.get("material_id"),
                  "machine_id": context.get("machine_id"), "generator_key": request.generator_key}
    ...
    logger.info("art_studio.workflow_from_generator", extra={"session_id": session.session_id,       "generator_key": request.generator_key,})
...
def create_workflow_from_snapshot(
    ...
    # Create session
    index_meta = {"tool_id": context.get("tool_id"), "material_id": context.get("material_id"),
                  "machine_id": context.get("machine_id"), "snapshot_id": request.snapshot_id,
    ...
    logger.info("art_studio.workflow_from_snapshot", extra={"session_id": session.session_id,       "snapshot_id": request.snapshot_id,})
...
def evaluate_session_feasibility(
    ...
    try:
        ...
        # Build RmosContext
        rmos_ctx = RmosContext(model_id=context_dict.get("model_id", "generic"),
            model_spec=context_dict.get("model_spec", {}), materials=material_profile)

        # Add design parameters to physics_inputs for calculator access
        rmos_ctx.physics_inputs = {"tool_diameter_mm": context_dict.get("tool", {}).get("diameter_mm", 6.0),
            ...
            "depth_of_cut_mm": design.get("depth_mm", 3.0), **design}

        # Evaluate using RMOS feasibility engine
        report = rmos_evaluate_feasibility(design, rmos_ctx)

        # Map RiskLevel to RiskBucket
        risk_level_to_bucket = {RiskLevel.GREEN: RiskBucket.GREEN, RiskLevel.YELLOW: RiskBucket.YELLOW,
            RiskLevel.RED: RiskBucket.RED, RiskLevel.UNKNOWN: RiskBucket.UNKNOWN}

        score = report.overall_score
        risk_bucket = risk_level_to_bucket.get(report.overall_risk, RiskBucket.UNKNOWN)

        # Collect warnings from assessments + recommendations
        warnings: List[str] = [w for a in report.assessments for w in a.warnings] + list(report.recommendations)
        ...
        outer_d, inner_d = design.get("outer_diameter_mm", 0), design.get("inner_diameter_mm", 0)
        if outer_d <= 0: warnings.append("Invalid outer diameter"); score -= 20
        if inner_d < 0: warnings.append("Invalid inner diameter"); score -= 10
        if outer_d > 0 and outer_d <= inner_d: warnings.append("Outer diameter must be > inner"); score -= 30; risk_bucket = RiskBucket.RED
        score = max(0.0, min(100.0, score))
        if score < 50: risk_bucket = RiskBucket.RED
        elif score < 70: risk_bucket = RiskBucket.YELLOW
    
    # Create feasibility result
    result = FeasibilityResult(score=score, risk_bucket=risk_bucket, warnings=warnings,
        meta={"source": "rmos_feasibility_fusion", "design_hash": str(hash(str(design)))})
    ...
    logger.info("art_studio.feasibility_evaluated", extra={"session_id": session_id,       "score": score,       "risk_bucket": risk_bucket.value,})
    
    return FeasibilityResponse(session_id=session_id, state=session.state.value, score=score,
        risk_bucket=risk_bucket.value, warnings=warnings,
        run_artifact_id=artifact_ref.artifact_id if artifact_ref else None, next_step=next_step_hint(session))
...
def approve_session(
    ...
    try:
        ...
        logger.info("art_studio.session_approved", extra={"session_id": session_id})
        
        return ApprovalResponse(session_id=session_id, state=session.state.value, approved=True,
            message="Session approved for toolpath generation", run_artifact_id=artifact_ref.artifact_id if artifact_ref else None)
    except (ValueError, PermissionError) as e:  # WP-1: narrowed from except Exception
        return ApprovalResponse(session_id=session_id, state=session.state.value, approved=False, message=str(e))
...
def reject_session(
    ...
    logger.info("art_studio.session_rejected", extra={"session_id": session_id, "reason": request.reason})
    
    return ApprovalResponse(session_id=session_id, state=session.state.value, approved=False,
        message=f"Session rejected: {request.reason}", run_artifact_id=artifact_ref.artifact_id if artifact_ref else None)
...
def request_session_revision(
    ...
    logger.info("art_studio.revision_requested", extra={"session_id": session_id, "reason": request.reason})
...
def update_session_design(
    ...
    logger.info("art_studio.design_updated", extra={"session_id": session_id})
...
def save_session_as_snapshot(
    ...
    context_refs = DesignContextRefs(material_preset_id=session.index_meta.get("material_id"),
        tool_preset_id=session.index_meta.get("tool_id"), machine_id=session.index_meta.get("machine_id"),
    ...
    if session.feasibility:
        feasibility_dict = {"score": session.feasibility.score, "risk_bucket": session.feasibility.risk_bucket.value,
            "warnings": session.feasibility.warnings, "meta": session.feasibility.meta}
    
    snapshot_request = SnapshotCreateRequest(name=request.name, notes=request.notes,
        pattern_id=session.index_meta.get("pattern_id"), tags=request.tags, context_refs=context_refs,
        rosette_params=rosette_params, feasibility=feasibility_dict)
    
    store = _get_snapshot_store()
    snapshot = store.create(snapshot_request)
    
    logger.info("art_studio.snapshot_saved", extra={"session_id": session_id,       "snapshot_id": snapshot.snapshot_id,})
    
    return SnapshotCreatedResponse(snapshot_id=snapshot.snapshot_id, name=snapshot.name, session_id=session_id)
...
def _session_to_status(session: WorkflowSession) -> SessionStatusResponse:
    ...
    f = session.feasibility
    feas_score, feas_risk, feas_warnings = (f.score, f.risk_bucket.value, f.warnings) if f else (None, None, [])
    
    return SessionStatusResponse(
        session_id=session.session_id, mode=session.mode.value, state=session.state.value,
        next_step=next_step_hint(session), events_count=len(session.events),
        has_design=session.design is not None, has_context=session.context is not None,
        feasibility_score=feas_score, feasibility_risk=feas_risk, feasibility_warnings=feas_warnings,
        last_feasibility_artifact_id=session.last_feasibility_artifact.artifact_id if session.last_feasibility_artifact else None,
        last_toolpaths_artifact_id=session.last_toolpaths_artifact.artifact_id if session.last_toolpaths_artifact else None,
        allow_red_override=session.allow_red_override, require_explicit_approval=session.require_explicit_approval)

def get_available_generators() -> List[GeneratorInfo]:
    """List available parametric generators."""
    descriptors = list_generators()
    
    return [GeneratorInfo(generator_key=d.generator_key, name=d.name, description=d.description, param_hints=d.param_hints)
