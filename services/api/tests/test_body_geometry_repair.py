"""
Tests for Body Geometry Repair — Phase 6A Analysis
===================================================

Tests conversion, contour run detection, arc fitting validation,
and deviation analysis. No POLYLINE output tests (removed per CLAUDE.md).
"""
...
# ─── Contour Run Detection Tests ──────────────────────────────────────────────

class TestContourRunDetection:
    """Test contour run detection."""
    ...
    def test_corner_breaks_run(self):
        """90-degree corner breaks contour into multiple runs."""
        # L-shaped path
        points = [(0, 0), (10, 0), (20, 0), (30, 0), (30, 10), (30, 20), (30, 30)]
        chain = make_chain(points)

        runs = detect_contour_runs(chain, dist_tol_mm=15.0, angle_tol_deg=45.0)
    ...
    def test_gentle_curve_single_run(self):
        ...
        runs = detect_contour_runs(chain, dist_tol_mm=50.0, angle_tol_deg=20.0)
...
class TestRepairBodyGeometry:
    ...
    def test_metrics_computed(self, monkeypatch):
        """Metrics include contour run detection results."""
        monkeypatch.setenv("ENABLE_BODY_REPAIR", "1")
        ...
        assert result.metrics.contour_runs_detected >= 1
    ...
    def test_result_to_dict(self, monkeypatch):
        ...
        assert "phase" in d
        assert "metrics" in d
        # Note: dxf_output_changed removed — Phase 6A is metrics-only
...
class TestRunAcceptance:
    """Test contour run acceptance evaluation."""

    def test_straight_line_accepted(self):
        """Straight line run is accepted."""
        points = make_line_points(0, 0, 100, 0, n=10)
        run = ContourRun(
            points=points,
            start_idx=0,
        ...
        evaluated = evaluate_run_acceptance(run, mean_tol_mm=0.5, max_tol_mm=1.5)
    ...
    def test_arc_rejected_with_strict_tolerance(self):
        """Arc run rejected with strict tolerance."""
        points = make_arc_points(0, 0, 50, 0, 90, n=10)
        run = ContourRun(
            points=points,
            start_idx=0,
        ...
        evaluated = evaluate_run_acceptance(run, mean_tol_mm=0.1, max_tol_mm=0.2)
    ...
    def test_gentle_arc_may_be_accepted(self):
        ...
        evaluated = evaluate_run_acceptance(run, mean_tol_mm=2.0, max_tol_mm=5.0)

        # Small arc deviation should be under relaxed tolerance
        assert evaluated.mean_deviation_mm < 2.0


# TestPhase6BIntegration removed — POLYLINE output is prohibited per CLAUDE.md
# Phase 6A analysis tests are in TestRepairBodyGeometry and TestPositionalDeviation
