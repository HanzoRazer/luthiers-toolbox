...
def test_batch_links_endpoint_returns_latest_ids(client: TestClient):
    # Create a chain
    spec = client.post(
        "/api/saw/batch/spec",
        json={
            "batch_label": "pytest-batch-links",
            "session_id": "sess_pytest_batch_links",
    ...
    # Links endpoint
    links = client.get("/api/saw/batch/links", params={"batch_label": "pytest-batch-links", "session_id": "sess_pytest_batch_links"})
    assert links.status_code == 200, links.text
    body = links.json()

    assert body["latest_spec_artifact_id"] == spec_id
    assert body["latest_plan_artifact_id"] == plan_id
    assert body["latest_decision_artifact_id"] == decision_id
    assert body["latest_execution_artifact_id"] == execution_id
