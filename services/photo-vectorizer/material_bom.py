"""
Material BOM Generator — Bill of Materials from Photo Extraction Results
=========================================================================
...
Design decisions:
  ...
  - No grain direction enforcement here — that is a separate concern
...
class MaterialBOM:
    ...
    def summary(self) -> str:
        ...
        for ln in self.lines:
            bbox = f"{ln.bbox_w_mm:.0f}x{ln.bbox_h_mm:.0f}"
            rows.append(
                f"  {ln.feature_type:<22} {ln.material:<18} "
                f"{ln.area_sq_in:>12.4f} {bbox:>14} {ln.cost_usd:>10.4f}")
        rows.append("  " + "-" * 80)
        rows.append(f"  {'TOTAL':>56} {self.total_cost_usd:>10.4f} USD")
        if self.warnings:
            rows.append("")
...
class MaterialBOMGenerator:
    ...
    def generate(self, extraction_result) -> MaterialBOM:
        ...
        for mat, group in material_groups.items():
            ...
            layout = self._nest_rectangles(mat, group)
            if layout:
