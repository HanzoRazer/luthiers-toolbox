...
def _arc_center_from_r(
    ...
    Args:
        ...
        ex, ey: End XY position
        r: Arc radius (positive = minor arc, negative = major arc)
        cw: True for clockwise (G2)

    Returns:
        (cx, cy) center, or None if radius is geometrically invalid.
    """
    dx, dy = ex - sx, ey - sy
    ...
    # Pick the center that gives the correct arc size:
    # R > 0 → minor arc (|sweep| ≤ π), R < 0 → major arc (|sweep| > π)
...
def _interpolate_arc_points(
    ...
    Each waypoint is a (x, y, z) tuple. Z is linearly interpolated
    from from_pos[2] to to_pos[2] across the sweep.

    Args:
        from_pos: Start position [x, y, z]
        to_pos:   End position [x, y, z]
        center:   Arc center (cx, cy)
        cw:       True = clockwise (G2)
        arc_resolution_deg: Angular step size in degrees

    Returns:
        List of waypoint tuples, ending exactly at to_pos.
    """
    sx, sy, sz = from_pos
    ex, ey, ez = to_pos
    cx, cy = center
    ...
    # Determine sweep direction and magnitude
...
def simulate_segments(
    ...
    *,
    ...
    Args:
        gcode: G-code program text
        rapid_mm_min: Rapid traverse rate (G0) in mm/min
        default_feed_mm_min: Default cutting feed if F not yet specified
        units: Input units ("mm" or "inch")
        arc_resolution_deg: Angular step for arc interpolation (degrees)

    Returns:
        Dict with:
        - segments: list of MoveSegment dicts
            {type, from_pos, to_pos, feed, duration_ms, line_number, line_text}
        - bounds: {x_min, x_max, y_min, y_max, z_min, z_max}
        - totals: {rapid_mm, cut_mm, time_min, segment_count}

    Invariant:
        sum(seg["duration_ms"] for seg in result["segments"]) / 60000
        ≈ simulate(gcode, ...)["t_total_min"]   (within floating-point tolerance)

    Example:
        >>> r = simulate_segments("G0 X10 Y0\\nG1 X20 F600")
        >>> r["segments"][0]["type"]
        'rapid'
        >>> r["segments"][1]["type"]
        'cut'
    ...
    # Bounding box — initialised from origin, expanded as points are emitted
    bb: Dict[str, float] = {
        "x_min": 0.0, "x_max": 0.0,
        "y_min": 0.0, "y_max": 0.0,
        "z_min": 0.0, "z_max": 0.0,
